		...
		if (servletApi == null) {
			try {
				if (context.getMajorVersion() > 4) {
					logger.info("Using servlet API 6");
					servletApi = (ServletApi) Class.forName("eu.webtoolkit.jwt.ServletApi6").getDeclaredConstructor().newInstance();
				} else if (context.getMajorVersion() == 4) {

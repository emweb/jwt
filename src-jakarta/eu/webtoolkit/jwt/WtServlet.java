...
public abstract class WtServlet extends HttpServlet {
	...
	private List<WResource> removableResources = new ArrayList<WResource>();
	private final ReentrantReadWriteLock staticResourcesLock = new ReentrantReadWriteLock();
	...
	static final String Wt_strings_xml = "/eu/webtoolkit/jwt/wt_strings";
	static final String Wt_templates_xml = "/eu/webtoolkit/jwt/wt_templates";
	...
	public void init(ServletConfig config) throws ServletException {
		...
		String configFile = this.getInitParameter("jwt-config");
		if (configFile == null) {
			configFile = this.getServletContext().getInitParameter("jwt-config");
		}
	...
	void handleRequest(final HttpServletRequest request, final HttpServletResponse response) {
		...
		if (pathInfo != null) {
			...
			staticResourcesLock.readLock().lock();
			...
			try {
				for (WResource staticResource : staticResources) {
					...
					if (requestPath.equals(staticResourcePath)) {
						staticResourcesLock.readLock().unlock();
						unlocked = true;
						try {
	...
	void doHandleRequest(WebRequest request, WebResponse response) {
		String wtd = request.getParameter("wtd");
		String path = request.getPathInfo();
		if ((wtd != null && configuration.agentIsBot(request.getUserAgent())) ||
				(path != null && path.startsWith("/"+configuration.getBotResourcesPath()))) {
			logger.debug("Not handling this request for bots");
			response.setStatus(404);
			response.flush();
			return;
		}
	...
	public void addResource(WResource staticResource, String path) {
		...
		staticResourcesLock.writeLock().lock();
		...
		try {
			...
			if (staticResource.isAllowAutoRemoval()) {
				int maxRemovableResources = configuration.getMaxAutoRemovablePublicResources();

				if (maxRemovableResources != 0) {
					removableResources.add(staticResource);
					staticResources.add(staticResource);
				}

				if (maxRemovableResources > 0 && removableResources.size() > maxRemovableResources) {
					removeResource(removableResources.get(0));
				}
			} else {
				staticResources.add(staticResource);
			}
		} finally {
			staticResourcesLock.writeLock().unlock();
		}
	}

	public void removeResource(WResource staticResource)
	{
		final Set<WResource> toRemove = Collections.singleton(staticResource);

		staticResourcesLock.writeLock().lock();

		try {
			if (staticResource != null && staticResource.isAllowAutoRemoval()) {
				removableResources.removeAll(toRemove);
			}

			staticResources.removeAll(toRemove);
		} finally {
			staticResourcesLock.writeLock().unlock();
		}
	}

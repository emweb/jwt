 ...
 *
 *  Modified by  Koen Deforche <koen@emweb.be> to:
 *   - report all whitespace
 *   - not resolve char literals or entities, as an option
 */
...
{
	public static boolean convertNumericalCharacterEntities = false;
    ...
    *		if an error occurred reading the data
    */
   static String processCharLiteral(String entity)
      throws IOException,
             XMLParseException
   {
	  if (!convertNumericalCharacterEntities)
		  return entity;

      String result = new String();
	  if (entity.charAt(2) == 'x') {
         entity = entity.substring(3, entity.length() - 1);
         return result += (char) Integer.parseInt(entity, 16);
      } else {
         entity = entity.substring(2, entity.length() - 1);
         return result += (char) Integer.parseInt(entity, 10);
      }
   }

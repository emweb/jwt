	...
	public WDialog(CharSequence windowTitle) {
		...
		if (!app.getStyleSheet().isDefined(CSS_RULES_NAME)) {
			...
			app
					.doJavaScript(
							""
									+ "Wt3_1_1.centerDialog = function(d){if (d && d.style.display != 'none') {d.style.visibility = 'visible';if (!d.getAttribute('moved')) {var ws=Wt3_1_1.windowSize();d.style.left=Math.round((ws.x - d.clientWidth)/2"

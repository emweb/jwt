 ...
 * Thus, you need to call {@link WMediaPlayer#addSource(WMediaPlayer.Encoding encoding, WLink link)
 ...
 * media player using the {@link WMediaPlayer#setButton(WMediaPlayer.ButtonControlId id,
 * WInteractWidget w) setButton()}, {@link WMediaPlayer#setText(WMediaPlayer.TextId id, WText w)
 * setText()} and {@link WMediaPlayer#setProgressBar(WMediaPlayer.BarControlId id, WProgressBar w)
 * setProgressBar()} methods. The controls widget is integrated in the media player, and this has as
 * unique benefit (for a video player) that they may also be shown when the video player is
 * maximized.
 *
...
public class WMediaPlayer extends WCompositeWidget {
   ...
   * <p>
   *
   * @see WMediaPlayer#setButton(WMediaPlayer.ButtonControlId id, WInteractWidget w)
   ...
   * <p>
   *
   * @see WMediaPlayer#setProgressBar(WMediaPlayer.BarControlId id, WProgressBar w)
  ...
  public enum TextId {
    ...
    Duration,
    /** Displays the title set in {@link WMediaPlayer#setTitle(CharSequence title) setTitle()} */
    Title;
   ...
   */
  public WMediaPlayer(WMediaPlayer.MediaType mediaType, WContainerWidget parent) {
    super(parent);
   ...
   * Creates a new media player.
   *
   * <p>Calls {@link #WMediaPlayer(WMediaPlayer.MediaType mediaType, WContainerWidget parent)
   ...
   * using {@link WMediaPlayer#setButton(WMediaPlayer.ButtonControlId id, WInteractWidget w)
   * setButton()}, {@link WMediaPlayer#setText(WMediaPlayer.TextId id, WText w) setText()} and
  ...
  public void setControlsWidget(WWidget controlsWidget) {
    if (this.gui_ != this) {
      if (this.gui_ != null) this.gui_.remove();
    }
  ...
  public void setTitle(final CharSequence title) {
    ...
    if (this.display_[WMediaPlayer.TextId.Title.getValue()] != null) {
      this.display_[WMediaPlayer.TextId.Title.getValue()].setText(this.title_);
      if (this.gui_ != null) {
   ...
   * been added using {@link WMediaPlayer#addSource(WMediaPlayer.Encoding encoding, WLink link)
   * addSource()}.
   */
  ...
  public void setButton(WMediaPlayer.ButtonControlId id, WInteractWidget w) {
    if (this.control_[id.getValue()] != null) this.control_[id.getValue()].remove();
   ...
   *
   * <p>The progress bar for the {@link WMediaPlayer.BarControlId#Time Time} indication should be
  ...
  public void setProgressBar(WMediaPlayer.BarControlId id, WProgressBar w) {
    ...
    if (this.progressBar_[id.getValue()] != null) this.progressBar_[id.getValue()].remove();
  ...
  public void setText(WMediaPlayer.TextId id, WText w) {
    if (this.display_[id.getValue()] != null) this.display_[id.getValue()].remove();
  ...
  protected void render(EnumSet<RenderFlag> flags) {
    ...
    if (this.mediaUpdated_
        || !EnumUtils.mask(flags, RenderFlag.RenderFull).isEmpty() && !this.media_.isEmpty()) {
      StringBuilder ss = new StringBuilder();
      ss.append('{');
      boolean first = true;
      for (int i = 0; i < this.media_.size(); ++i) {
    ...
    if (!EnumUtils.mask(flags, RenderFlag.RenderFull).isEmpty()) {
      ...
      first = true;
      for (int i = WMediaPlayer.ButtonControlId.VideoPlay.getValue();
          i < WMediaPlayer.ButtonControlId.RepeatOff.getValue();
          ++i) {
      ...
      for (int i = WMediaPlayer.TextId.CurrentTime.getValue();
          i < WMediaPlayer.TextId.Duration.getValue();
          ++i) {
        if (this.control_[i] != null) {
          if (!first) {
            ss.append(", ");
      ...
      if (this.progressBar_[WMediaPlayer.BarControlId.Time.getValue()] != null) {
        ...
        ss.append("seekBar:\"#")
            .append(this.progressBar_[WMediaPlayer.BarControlId.Time.getValue()].getId())
            .append("\", ")
            .append("playBar:\"#bar")
            .append(this.progressBar_[WMediaPlayer.BarControlId.Time.getValue()].getId())
            .append("\"");
      ...
      if (this.progressBar_[WMediaPlayer.BarControlId.Volume.getValue()] != null) {
        ...
        ss.append("volumeBar:\"#")
            .append(this.progressBar_[WMediaPlayer.BarControlId.Volume.getValue()].getId())
            .append("\", ")
      ...
      ss.append('}').append("});");
      ss.append("new Wt3_7_0.WMediaPlayer(")
  ...
  private void createDefaultGui() {
    this.gui_ = null;
    WTemplate ui =
        new WTemplate(tr("Wt.WMediaPlayer.defaultgui-" + media[this.mediaType_.getValue()]));
    this.addAnchor(ui, WMediaPlayer.ButtonControlId.Play, "play-btn", "jp-play");
    this.addAnchor(ui, WMediaPlayer.ButtonControlId.Pause, "pause-btn", "jp-pause");
    this.addAnchor(ui, WMediaPlayer.ButtonControlId.Stop, "stop-btn", "jp-stop");
    this.addAnchor(ui, WMediaPlayer.ButtonControlId.VolumeMute, "mute-btn", "jp-mute");
    this.addAnchor(ui, WMediaPlayer.ButtonControlId.VolumeUnmute, "unmute-btn", "jp-unmute");
    this.addAnchor(ui, WMediaPlayer.ButtonControlId.VolumeMax, "volume-max-btn", "jp-volume-max");
    this.addAnchor(ui, WMediaPlayer.ButtonControlId.RepeatOn, "repeat-btn", "jp-repeat");
    this.addAnchor(ui, WMediaPlayer.ButtonControlId.RepeatOff, "repeat-off-btn", "jp-repeat-off");
    if (this.mediaType_ == WMediaPlayer.MediaType.Video) {
      this.addAnchor(
          ui,
          WMediaPlayer.ButtonControlId.VideoPlay,
          "video-play-btn",
          "jp-video-play-icon",
          "play");
      this.addAnchor(
          ui, WMediaPlayer.ButtonControlId.FullScreen, "full-screen-btn", "jp-full-screen");
      this.addAnchor(
          ui,
          WMediaPlayer.ButtonControlId.RestoreScreen,
          "restore-screen-btn",
          "jp-restore-screen");
    }
    this.addText(ui, WMediaPlayer.TextId.CurrentTime, "current-time", "jp-current-time");
    this.addText(ui, WMediaPlayer.TextId.Duration, "duration", "jp-duration");
    this.addText(ui, WMediaPlayer.TextId.Title, "title", "");
    this.addProgressBar(
        ui, WMediaPlayer.BarControlId.Time, "progress-bar", "jp-seek-bar", "jp-play-bar");
    this.addProgressBar(
        ui, WMediaPlayer.BarControlId.Volume, "volume-bar", "jp-volume-bar", "jp-volume-bar-value");
    ui.bindString("title-display", (this.title_.length() == 0) ? "none" : "");
    this.addStyleClass(this.mediaType_ == WMediaPlayer.MediaType.Video ? "jp-video" : "jp-audio");
    this.setControlsWidget(ui);
  }

  private void addAnchor(
      WTemplate t,
      WMediaPlayer.ButtonControlId id,
      String bindId,
    ...
    WAnchor anchor = new WAnchor(new WLink("javascript:;"), WString.tr(text));
  ...
  private final void addAnchor(
      WTemplate t, WMediaPlayer.ButtonControlId id, String bindId, final String styleClass) {
    addAnchor(t, id, bindId, styleClass, "");
  }

  private void addText(
      WTemplate t, WMediaPlayer.TextId id, String bindId, final String styleClass) {
  ...
  private void updateFromProgressBar(WMediaPlayer.BarControlId id, double value) {
    switch (id) {
      case Time:
        this.seek(value);
        break;

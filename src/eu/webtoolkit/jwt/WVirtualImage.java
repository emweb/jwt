...
public class WVirtualImage extends WCompositeWidget {
  ...
  public void enableDragging() {
    this.impl_
        .mouseWentDown()
        .addListener(
            "function(obj, event) {  var pc = Wt4_10_0.pageCoordinates(event);  obj.setAttribute('dsx', pc.x);  obj.setAttribute('dsy', pc.y);}");
    this.impl_
        .mouseMoved()
        .addListener(
            "function(obj, event) {var WT= Wt4_10_0;var lastx = obj.getAttribute('dsx');var lasty = obj.getAttribute('dsy');if (lastx != null && lastx != '') {var nowxy = WT.pageCoordinates(event);var img = "
   ...
   *
   * <p>The default implementation calls {@link WVirtualImage#render(long x, long y, int width, int
   * height) render()} and creates an image for the resource returned.
   *

 ...
 * model, and presents methods to organize columns using aggregation, through
...
public class WAggregateProxyModel extends WAbstractProxyModel {
	 ...
	 * Constructor.
	 * <p>
	 * Calls {@link #WAggregateProxyModel(WObject parent) this((WObject)null)}
	 */
	 ...
	 * <p>
	 * Only one aggregate can be defined per <code>parentColumn</code>.
	 */
	public void addAggregate(int parentColumn, int firstColumn, int lastColumn) {
		WAggregateProxyModel.Aggregate added = this.topLevel_
				.add(new WAggregateProxyModel.Aggregate(parentColumn,
		...
		this.modelConnections_.add(this.getSourceModel().columnsRemoved()
				.addListener(this,
						new Signal3.Listener<WModelIndex, Integer, Integer>() {
							public void trigger(WModelIndex e1, Integer e2,
									Integer e3) {
								WAggregateProxyModel.this.sourceColumnsRemoved(
										e1, e2, e3);
							}
						}));
		this.modelConnections_.add(this.getSourceModel()
		...
		this.modelConnections_.add(this.getSourceModel().rowsInserted()
				.addListener(this,
						new Signal3.Listener<WModelIndex, Integer, Integer>() {
							public void trigger(WModelIndex e1, Integer e2,
									Integer e3) {
								WAggregateProxyModel.this.sourceRowsInserted(
										e1, e2, e3);
							}
						}));
		...
		this.modelConnections_.add(this.getSourceModel().rowsRemoved()
				.addListener(this,
						new Signal3.Listener<WModelIndex, Integer, Integer>() {
							public void trigger(WModelIndex e1, Integer e2,
									Integer e3) {
								WAggregateProxyModel.this.sourceRowsRemoved(e1,
										e2, e3);
							}
						}));
				...
				.addListener(this,
						new Signal2.Listener<WModelIndex, WModelIndex>() {
							public void trigger(WModelIndex e1, WModelIndex e2) {
								WAggregateProxyModel.this.sourceDataChanged(e1,
										e2);
							}
						}));
	...
	public void expandColumn(int column) {
		...
		WAggregateProxyModel.Aggregate ag = this.topLevel_
				.findAggregate(sourceColumn);
		if (ag != null) {
			this.expand(ag);
		}
	}
	...
	public Object getHeaderData(int section, Orientation orientation, int role) {
		if (orientation == Orientation.Horizontal) {
			...
			if (role == ItemDataRole.LevelRole) {
				WAggregateProxyModel.Aggregate agg = this.topLevel_
						.findEnclosingAggregate(section);
				return agg.level_;
			} else {
				return this.getSourceModel().getHeaderData(section,
						orientation, role);
			}
		} else {
	...
	static class Aggregate {
		...
		public boolean contains(int sourceColumn) {
			return this.firstChildSrc_ <= sourceColumn
					&& sourceColumn <= this.lastChildSrc_;
		}
		...
		public WAggregateProxyModel.Aggregate add(
			...
			return this.nestedAggregates_
					.get(this.nestedAggregates_.size() - 1);
		}
				...
				if (a.after(sourceColumn)) {
					...
					if (a.contains(sourceColumn)) {
						if (a.collapsed_) {
							...
							return a.mapFromSource(sourceColumn)
									- collapsedCount;
						}
					} else {
	...
	private void expand(WAggregateProxyModel.Aggregate aggregate) {
		...
		if (c >= 0) {
			...
			int c1 = this.topLevel_.mapFromSource(this
					.firstVisibleSourceNotBefore(aggregate.firstChildSrc_));
			int c2 = this.topLevel_.mapFromSource(this
					.lastVisibleSourceNotAfter(aggregate.lastChildSrc_));
	...
	private void propagateBeginRemove(WModelIndex proxyIndex, int start, int end) {
		...
		for (int i = 0; i < rc; ++i) {
			this.propagateBeginRemove(this.getIndex(i, 0, proxyIndex), start,
					end);
		}
	}
	...
	private void propagateBeginInsert(WModelIndex proxyIndex, int start, int end) {
		...
		int rc = this.getRowCount(proxyIndex);
		for (int i = 0; i < rc; ++i) {
			this.propagateBeginInsert(this.getIndex(i, 0, proxyIndex), start,
					end);
		}
	}
	...
	private void sourceDataChanged(WModelIndex topLeft, WModelIndex bottomRight) {
		...
		if (r >= l) {
			WModelIndex tl = this.mapFromSource(this.getSourceModel().getIndex(
					topLeft.getRow(), l, topLeft.getParent()));
			WModelIndex br = this.mapFromSource(this.getSourceModel().getIndex(
					bottomRight.getRow(), r, bottomRight.getParent()));

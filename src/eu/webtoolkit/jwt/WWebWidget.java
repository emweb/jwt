...
public abstract class WWebWidget extends WWidget {
	...
	void updateDom(final DomElement element, boolean all) {
				...
				if (this.layoutImpl_.zIndex_ > 0) {
					...
					if (all
						...
						app.addAutoJavaScript("{var w = "
								+ this.getJsRef()
								+ ";if (w && !Wt3_5_0.isHidden(w)) {var i = Wt3_5_0.getElement('"
								+ i.getId()
				...
				if (!all
					...
					if ((this.lookImpl_.toolTipTextFormat_ != TextFormat.PlainText || this.flags_
						...
						String deferred = this.flags_.get(BIT_TOOLTIP_DEFERRED) ? "true"
								: "false";
						element.callJavaScript("Wt3_5_0.toolTip("
		...
		if (all || this.flags_.get(BIT_SCROLL_VISIBILITY_CHANGED)) {
			...
			if (!app.isJavaScriptLoaded(SCROLL_JS)
				...
				StringBuilder ss = new StringBuilder();
				ss.append("if (!Wt3_5_0.scrollVisibility) {Wt3_5_0.scrollVisibility = new ");
				ss.append("Wt3_5_0.ScrollVisibility(").append(
						app.getJavaScriptClass() + "); }");
				element.callJavaScript(ss.toString());
			}
			if (this.isScrollVisibilityEnabled()) {
				StringBuilder ss = new StringBuilder();
				ss.append("Wt3_5_0.scrollVisibility.add({");
				...
				if (this.flags_.get(BIT_SCROLL_VISIBILITY_LOADED)) {
					element.callJavaScript("Wt3_5_0.scrollVisibility.remove("
							+ jsStringLiteral(this.getId()) + ");");
	...
	String renderRemoveJs(boolean recursive) {
		...
		if (this.isRendered() && this.isScrollVisibilityEnabled()) {
			result += "Wt3_5_0.scrollVisibility.remove("
					+ jsStringLiteral(this.getId()) + ");";
			this.flags_.set(BIT_SCROLL_VISIBILITY_CHANGED);
			this.flags_.clear(BIT_SCROLL_VISIBILITY_LOADED);
		}
		...
		if (!recursive) {
			if (result.length() == 0) {
				...
				result += "Wt3_5_0.remove('" + this.getId() + "');";
			}
		}
		return result;
	}
	...
	static WJavaScriptPreamble wtjs10() {
		return new WJavaScriptPreamble(
				...
				JavaScriptObjectType.JavaScriptFunction,
				"toolTip",
				"function(m,n,o,r,s,t){var a=document.getElementById(n),p=m.WT,i=a.toolTip;if(!i)a.toolTip=new (function(){function u(){$(\"#\"+n+\":hover\").length||j()}function v(){e=true;m.emit(a,\"Wt-loadToolTip\")}function j(){clearTimeout(f);setTimeout(function(){if(!k)if(b){$(b).parent().remove();b=null;clearInterval(d);d=null}},w)}function g(c){clearTimeout(f);l=p.pageCoordinates(c);b||(f=setTimeout(function(){a.toolTip.showToolTip()},x))}var f=null,d=null,l= null,b=null,x=500,w=200,e=false,h=o,k=false;this.setToolTipText=function(c){h=c;if(e){this.showToolTip();clearTimeout(f);e=false}};this.showToolTip=function(){r&&!h&&!e&&v();if(h){b=document.createElement(\"div\");b.className=s;b.innerHTML=h;outerDiv=document.createElement(\"div\");outerDiv.className=t;document.body.appendChild(outerDiv);outerDiv.appendChild(b);var c=l.x,q=l.y;p.fitToWindow(outerDiv,c+10,q+10,c-10,q-10);$(b).mouseenter(function(){k=true});$(b).mouseleave(function(){k=false})}clearInterval(d); d=null;d=setInterval(function(){u()},200)};if(a.addEventListener){a.addEventListener(\"mouseenter\",g);a.addEventListener(\"mousemove\",g);a.addEventListener(\"mouseleave\",j)}else{a.attachEvent(\"mouseenter\",g);a.attachEvent(\"mousemove\",g);a.attachEvent(\"mouseleave\",j)}});i&&i.setToolTipText(o)}");
	}

...
class StdGridLayoutImpl extends StdLayoutImpl {
	...
	static WJavaScriptPreamble appjs1() {
		return new WJavaScriptPreamble(
				...
				"layouts",
				"new (function(){var a=[],r=false;this.add=function(j){var o,b;o=0;for(b=a.length;o<b;++o){var d=a[o];if(d.getId()==j.getId()){a[o]=j;return}else if(j.contains(d)){a.splice(o,0,j);return}}a.push(j)};this.adjust=function(j){if(j){if(j=$(\"#\"+j).get(0))j.dirty=true}else if(!r){r=true;for(var o=0;o<a.length;++o){j=a[o];if(!j.adjust()){j.WT.arrayRemove(a,o);--o}}r=false}}})");
	}

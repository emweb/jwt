...
public class WPanel extends WCompositeWidget {
  ...
  public WPanel(WContainerWidget parentContainer) {
    ...
    this.setJavaScriptMember(
        WT_RESIZE_JS,
        "function(self, w, h, s) {var hdefined = h >= 0;if (hdefined) {var mh = Wt4_9_2.px(self, 'maxHeight');if (mh > 0) h = Math.min(h, mh);}if (Wt4_9_2.boxSizing(self)) {h -= Wt4_9_2.px(self, 'borderTopWidth') + Wt4_9_2.px(self, 'borderBottomWidth');}var c = self.lastChild;var t = c.previousSibling;if (t)h -= t.offsetHeight;h -= 8;if (hdefined && h > 0) {c.lh = true;c.style.height = h + 'px';c.querySelectorAll(':scope > *').forEach(function(self) { let padding = self.getBoundingClientRect().height - Wt4_9_2.px(self, 'height');self.style.height = (h - padding) + 'px';self.lh = true;});} else {c.style.height = '';c.lh = false;for (const child of c.children) {child.style.height = '';child.lh = false;}}};");

 ...
 * <p>
 * 
 * A {@link WIdentityProxyModel} simply forwards the structure of the source
 * model, without any transformation. {@link WIdentityProxyModel} can be used as
 * a base class for implementing proxy models that reimplement
 * {@link WAbstractProxyModel#getData(WModelIndex index, int role)
 * WAbstractProxyModel#getData()}, but retain all other characteristics of the
 * source model.
 */
public class WIdentityProxyModel extends WAbstractProxyModel {
	private static Logger logger = LoggerFactory
			.getLogger(WIdentityProxyModel.class);
	 ...
	 * Constructor.
	 * <p>
	 * Calls {@link #WIdentityProxyModel(WObject parent) this((WObject)null)}
	 */
	...
	public WModelIndex getIndex(int row, int column, final WModelIndex parent) {
		...
		final WModelIndex sourceIndex = this.getSourceModel().getIndex(row,
				column, sourceParent);
	...
	public WModelIndex mapFromSource(final WModelIndex sourceIndex) {
		...
		return this.createIndex(sourceIndex.getRow(), sourceIndex.getColumn(),
	...
	public void setSourceModel(WAbstractItemModel newSourceModel) {
		...
		if (newSourceModel != null) {
							...
							.addListener(
									...
									new Signal3.Listener<WModelIndex, Integer, Integer>() {
										public void trigger(WModelIndex e1,
												Integer e2, Integer e3) {
											WIdentityProxyModel.this
							...
							.addListener(
									...
									new Signal3.Listener<WModelIndex, Integer, Integer>() {
										public void trigger(WModelIndex e1,
												Integer e2, Integer e3) {
											WIdentityProxyModel.this
													.sourceColumnsRemoved(e1,
															e2, e3);
										}
									}));
			this.modelConnections_.add(newSourceModel.modelReset().addListener(
					this, new Signal.Listener() {
						public void trigger() {
							WIdentityProxyModel.this.sourceModelReset();
						}
					}));
			this.modelConnections_.add(newSourceModel.dataChanged()
					.addListener(this,
							new Signal2.Listener<WModelIndex, WModelIndex>() {
								public void trigger(WModelIndex e1,
										WModelIndex e2) {
									WIdentityProxyModel.this.sourceDataChanged(
											e1, e2);
								}
							}));
			this.modelConnections_
					.add(newSourceModel
							...
							.addListener(
									...
									new Signal3.Listener<Orientation, Integer, Integer>() {
										public void trigger(Orientation e1,
												Integer e2, Integer e3) {
											WIdentityProxyModel.this
			...
			this.modelConnections_.add(newSourceModel.layoutAboutToBeChanged()
			...
			this.modelConnections_.add(newSourceModel.layoutChanged()
					.addListener(this, new Signal.Listener() {
						public void trigger() {
	...
	public boolean insertColumns(int column, int count, final WModelIndex parent) {
		return this.getSourceModel().insertColumns(column, count,
	...
	public boolean insertRows(int row, int count, final WModelIndex parent) {
		return this.getSourceModel().insertRows(row, count,
				this.mapToSource(parent));
	}
	...
	public boolean removeColumns(int column, int count, final WModelIndex parent) {
		return this.getSourceModel().removeColumns(column, count,
				this.mapToSource(parent));
	}
	...
	public boolean removeRows(int row, int count, final WModelIndex parent) {
		return this.getSourceModel().removeRows(row, count,
				this.mapToSource(parent));
	}
	...
	private void sourceDataChanged(final WModelIndex topLeft,
		...
		this.dataChanged().trigger(this.mapFromSource(topLeft),

 ...
 *
 * <p>Use a {@link WScrollArea} to add scrolling capabilities to another widget. When the content is
 * bigger than the {@link WScrollArea}, scrollbars are added so that the user can still view the
 * entire content.
 *
 * <p>Use {@link WScrollArea#setScrollBarPolicy(WScrollArea.ScrollBarPolicy policy)
 * setScrollBarPolicy()} to configure if and when the scrollbars may appear.
 *
 * <p>In many cases, it might be easier to use the CSS overflow property on a {@link
 * WContainerWidget} (see {@link WContainerWidget#setOverflow(WContainerWidget.Overflow value,
 * EnumSet orientation) WContainerWidget#setOverflow()}). However, this class will behave better
 ...
 * <h3>CSS</h3>
 *
 * <p>This widget is rendered using a <code>&lt;div&gt;</code> with a CSS overflow attribute. When
...
public class WScrollArea extends WWebWidget {
   ...
   * Creates a scroll area.
   *
   * <p>Calls {@link #WScrollArea(WContainerWidget parent) this((WContainerWidget)null)}
   */
  public WScrollArea() {
   ...
   *
   * <p>Setting a new widget will delete the previously set widget.
   */
   ...
   * <p>
   *
   * @see WScrollArea#setHorizontalScrollBarPolicy(WScrollArea.ScrollBarPolicy policy)
   * @see WScrollArea#setVerticalScrollBarPolicy(WScrollArea.ScrollBarPolicy policy)
   */
  ...
  void updateDom(final DomElement element, boolean all) {
    if (this.widgetChanged_ || all) {
      if (this.widget_ != null) {
        element.addChild(this.widget_.createSDomElement(WApplication.getInstance()));
      }
      this.widgetChanged_ = false;
    }

...
class WebRenderer implements SlotLearnerInterface {
  ...
  public boolean checkResponsePuzzle(final WebRequest request) {
    if (this.solution_.length() != 0) {
      ...
      solution = new ArrayList<String>(Arrays.asList(this.solution_.split(",")));
  ...
  static class CookieValue {
    ...
    public CookieValue(final String v, final String p, final String d, final WDate e, boolean s) {
      this.value = v;
      this.path = p;
      this.domain = d;
      this.expires = e;
  ...
  private void setHeaders(final WebResponse response, final String mimeType) {
    for (Iterator<Map.Entry<String, WebRenderer.CookieValue>> i_it =
            this.cookiesToSet_.entrySet().iterator();
        i_it.hasNext(); ) {
      ...
      header
          .append(Utils.urlEncode(i.getKey()))
          .append('=')
          .append(Utils.urlEncode(value))
          .append("; Version=1;");
      if (!(cookie.expires == null)) {
        String formatString = "EEE, dd-MMM-yyyy HH:mm:ss 'GMT'";
        String d = cookie.expires.toString(new WString(formatString).toString(), false);
      ...
      if (cookie.path.length() == 0) {
        if (this.session_.getEnv().publicDeploymentPath_.length() != 0) {
          header.append(" Path=").append(this.session_.getEnv().publicDeploymentPath_).append(';');
        } else {
          header.append(" Path=").append(this.session_.getEnv().getDeploymentPath()).append(';');
        }
      } else {
  ...
  private void serveMainscript(final WebResponse response) throws IOException {
    ...
    if (serveSkeletons) {
      ...
      FileServe script = new FileServe(parts.size() > 1 ? Wt_js_combined : WtServlet.Wt_js);
      script.setCondition(
  ...
  private void serveMainAjax(final StringBuilder out) {
    ...
    if (!this.initialStyleRendered_) {
      if (app.getTheme() != null) {
        List<WCssStyleSheet> styleSheets = app.getTheme().getStyleSheets();
  ...
  private void loadStyleSheet(
      final StringBuilder out, WApplication app, final WCssStyleSheet sheet) {
    out.append("Wt3_5_1")
  ...
  private void renderStyleSheet(
      final StringBuilder out, final WCssStyleSheet sheet, WApplication app) {
  ...
  private String getHeadDeclarations() {
    ...
    if (this.session_.getApp() != null) {
      ...
      if (this.session_.getEnv().agentIsIE()) {
        if (this.session_.getEnv().getAgent().getValue() < WEnvironment.UserAgent.IE9.getValue()) {
  ...
  void updateMultiSessionCookie(final WebRequest request) {
    ...
    this.setCookie(
        ...
        WDate.getCurrentServerDate().addSeconds(conf.getMultiSessionCookieTimeout()),
        "",
        "",

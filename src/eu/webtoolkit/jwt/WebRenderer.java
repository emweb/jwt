...
class WebRenderer implements SlotLearnerInterface {
	...
	private void serveMainAjax(WebResponse response) throws IOException {
		...
		if (app.getCssTheme().length() != 0) {
			response.out().append("Wt3_1_7a").append(".addStyleSheet('")
					.append(WApplication.getResourcesUrl()).append("/themes/")
					.append(app.getCssTheme()).append("/wt.css', 'all');");
			if (app.getEnvironment().agentIsIE()) {
				response.out().append("Wt3_1_7a").append(".addStyleSheet('")
		...
		if (widgetset) {
			...
			if (historyE != null) {
				response.out().append("Wt3_1_7a").append(
	...
	private void loadStyleSheets(Writer out, WApplication app)
		...
		for (int i = first; i < app.styleSheets_.size(); ++i) {
			out.append("Wt3_1_7a").append(".addStyleSheet('").append(

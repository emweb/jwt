...
class WebRenderer implements SlotLearnerInterface {
	...
	private void serveMainscript(final WebResponse response) throws IOException {
		...
		if (serveSkeletons) {
			...
			script.setVar("WT_CLASS", "Wt3_5_1");
		...
		if (this.session_.getType() == EntryPointType.WidgetSet) {
			...
			if (!this.rendered_) {
				...
				if (app.enableAjax_) {
					this.collectedJS1_
							.append("var form = Wt3_5_1.getElement('Wt-form'); if (form) {")
					...
					this.collectedJS1_.append("var domRoot=")
							...
							.append("Wt3_5_1.progressed(domRoot);");
					...
					this.collectedJS2_
							.append("Wt3_5_1.resolveRelativeAnchors();")
	...
	private void serveMainAjax(final StringBuilder out) {
		...
		if (widgetset) {
			...
			if (historyE != null) {
				out.append("Wt3_5_1").append(".history.initialize('")
	...
	private void loadStyleSheet(final StringBuilder out, WApplication app,
		...
		out.append("Wt3_5_1").append(".addStyleSheet('")

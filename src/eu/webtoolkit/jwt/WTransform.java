...
public class WTransform extends WJavaScriptExposableObject {
	...
	public WPointF map(final WPointF p) {
		...
		if (this.isJavaScriptBound() || p.isJavaScriptBound()) {
			...
			result.assignBinding(o,
					"Wt3_3_10.gfxUtils.transform_mult(" + this.getJsRef() + ','
							+ p.getJsRef() + ')');
		}
		return result;
	}
	...
	public WPainterPath map(final WPainterPath path) {
		...
		if (this.isJavaScriptBound() || path.isJavaScriptBound()) {
			...
			result.assignBinding(o,
					"Wt3_3_10.gfxUtils.transform_apply(" + this.getJsRef()
							+ ',' + path.getJsRef() + ')');
		}
		...
		if (this.isJavaScriptBound() || p.isJavaScriptBound()) {
			...
			if (identity) {
				...
				this.assignBinding(o,
						"Wt3_3_10.gfxUtils.transform_mult((function(){var p="
								+ p.getJsRef()
								+ ";return [1,0,0,1,p[0],p[1]];})(),("
								+ refBefore + "))");
			}
	...
	public WTransform getAdjoint() {
		...
		if (this.isJavaScriptBound()) {
			res.assignBinding(this, "Wt3_3_10.gfxUtils.transform_adjoint("
					+ this.getJsRef() + ")");
		}
		return res;
	}
	...
	public WTransform getInverted() {
		...
		if (det != 0) {
			...
			WTransform res = new WTransform(adj.getM11() / det, adj.getM12()
					...
					/ det, adj.getM32() / det);
			if (this.isJavaScriptBound()) {
				res.assignBinding(this, "Wt3_3_10.gfxUtils.transform_inverted("
						+ this.getJsRef() + ")");
			}
			return res;
		} else {

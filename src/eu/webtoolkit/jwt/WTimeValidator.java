...
public class WTimeValidator extends WRegExpValidator {
	 ...
	 */
	public WTimeValidator(final String format, final WTime bottom,
			final WTime top, WObject parent) {
		super(parent);
		this.formats_ = new ArrayList<String>();
		this.bottom_ = bottom;
		this.top_ = top;
	 ...
	 * <p>
	 * Calls
	 * {@link #WTimeValidator(String format, WTime bottom, WTime top, WObject parent)
	...
	public WString getInvalidNotATimeText() {
		if (!(this.notATimeText_.length() == 0)) {
			...
			return WString.tr("Wt.WTimeValidator.WrongFormat").arg(
					this.formats_.get(0));
		}
	}
	...
	public WString getInvalidTooEarlyText() {
		if (!(this.tooEarlyText_.length() == 0)) {
			...
			if ((this.bottom_ == null)) {
				...
				if ((this.top_ == null)) {
					return WString.tr("Wt.WTimeValidator.TimeTooEarly").arg(
	...
	public WString getInvalidTooLateText() {
		if (!(this.tooLateText_.length() == 0)) {
			...
			if ((this.top_ == null)) {
				...
				if ((this.bottom_ == null)) {
					return WString.tr("Wt.WTimeValidator.TimeTooLate").arg(
							this.top_.toString(this.formats_.get(0)));
				} else {
					return WString.tr("Wt.WTimeValidator.WrongTimeRange")
							.arg(this.bottom_.toString(this.formats_.get(0)))
	...
	public WValidator.Result validate(final String input) {
		...
		return new WValidator.Result(WValidator.State.Invalid,
				this.getInvalidNotATimeText());
	}

	public String getJavaScriptValidate() {
		loadJavaScript(WApplication.getInstance());
		StringBuilder js = new StringBuilder();
		js.append("new Wt3_3_12.WTimeValidator(").append(this.isMandatory())
				.append(",[");
		for (int i = 0; i < this.formats_.size(); ++i) {
			...
			js.append("{").append("regexp:")
					...
					.append("getHour:function(results){").append(r.hourGetJS)
					.append(";},").append("getMinutes:function(results){")
					.append(r.minuteGetJS).append(";},")
					.append("getSeconds:function(results){").append(r.secGetJS)
					.append(";},").append("getMilliseconds:function(results){")
					.append(r.msecGetJS).append(";},").append("}");
		}
		js.append("],");
		if (!(this.bottom_ == null)) {
			js.append("new Date(0,0,0,").append(this.bottom_.getHour())
					.append(",").append(this.bottom_.getMinute()).append(",")
					.append(this.bottom_.getSecond()).append(",")
					.append(this.bottom_.getMsec()).append(")");
		} else {
			js.append("null");
		}
		js.append(',');
		if (!(this.top_ == null)) {
			js.append("new Date(0,0,0,").append(this.top_.getHour())
					.append(",").append(this.top_.getMinute()).append(",")
					.append(this.top_.getSecond()).append(",")
					.append(this.top_.getMsec()).append(")");
		} else {
			js.append("null");
		}
		js.append(',')
				.append(WString.toWString(this.getInvalidBlankText())
						.getJsStringLiteral())
				.append(',')
				.append(WString.toWString(this.getInvalidNotATimeText())
						.getJsStringLiteral())
				.append(',')

...
public class WTreeView extends WAbstractItemView {
	...
	public WTreeView(WContainerWidget parent) {
		...
		this.itemTouchEvent_ = new JSignal5<String, String, String, String, WTouchEvent>(
	...
	public void remove() {
		...
		if (this.rowWidthRule_ != null)
			this.rowWidthRule_.remove();
		if (this.rowContentsWidthRule_ != null)
			this.rowContentsWidthRule_.remove();
		if (this.c0StyleRule_ != null)
			this.c0StyleRule_.remove();
		this.impl_.clear();
		super.remove();
	}
			...
			if (node != null) {
				...
				if (w != null) {
					...
					this.renderedRowsChanged(
							this.renderedRow(index, spacer,
									this.getRenderLowerBound(),
									this.getRenderUpperBound()), diff);
				}
			}
		}
	}
	...
	public void scrollTo(final WModelIndex index,
		...
		if (app.getEnvironment().hasAjax()) {
			...
			s.append("setTimeout(function() { jQuery.data(")
					.append(this.getJsRef()).append(", 'obj').scrollTo(-1, ")
					.append(row).append(",")
		...
		if (!EnumUtils.mask(flags, RenderFlag.RenderFull).isEmpty()) {
			...
			if (!this.itemTouchEvent_.isConnected()) {
				this.itemTouchEvent_
						.addListener(
								this,
								new Signal5.Listener<String, String, String, String, WTouchEvent>() {
									public void trigger(String e1, String e2,
											String e3, String e4, WTouchEvent e5) {
										WTreeView.this.onItemTouchEvent(e1, e2,
												e3, e4, e5);
									}
								});
			}
			if (!this.itemEvent_.isConnected()) {
				...
				this.rowHeightRule_.getTemplateWidget().setHeight(
						this.getRowHeight());
				this.rowHeightRule_.getTemplateWidget().setLineHeight(
						this.getRowHeight());
				...
				this.c0StyleRule_ = this.addCssRule("#" + this.getId()
						+ " li .none",
				...
				if (this.columns_.size() > 0) {
					...
					this.c0StyleRule_.setSelector("#" + this.getId() + " li ."
							+ ci.getStyleClass());
				}
			}
		}
	...
	private JSignal5<String, String, String, String, WTouchEvent> itemTouchEvent_;
				...
				if (parentNode.isChildrenLoaded()) {
					...
					if (startWidget != null
						...
						if (renderedRowsChange) {
							this.renderedRowsChanged(
									this.renderedRow(
											this.getModel().getIndex(start, 0,
													parent),
											parentNode.topSpacer(),
											this.getRenderLowerBound(),
											this.getRenderUpperBound()), count);
						}
					} else {
						if (startWidget != null
								&& startWidget == parentNode.bottomSpacer()) {
							...
							int containerIndex = startWidget != null ? parentNode
									.getChildContainer()
									.getIndexOf(startWidget) : parentNode
									.getChildContainer().getCount();
							int parentRowCount = this.getModel().getRowCount(
									parent);
							int nodesToAdd = Math.max(0,
									Math.min(count, maxRenderHeight));
							...
							if (first != null && renderedRowsChange) {
								this.renderedRowsChanged(first.renderedRow(
										this.getRenderLowerBound(),
										this.getRenderUpperBound()), nodesToAdd);
							}
							if (end == this.getModel().getRowCount(parent) - 1
									&& start >= 1) {
								WTreeViewNode n = ((parentNode
										.widgetForModelRow(start - 1)) instanceof WTreeViewNode ? (WTreeViewNode) (parentNode
										.widgetForModelRow(start - 1)) : null);
								if (n != null) {
	...
	private void onItemTouchEvent(String nodeAndColumnId, String type,
			String extra1, String extra2, WTouchEvent event) {
	...
	private WModelIndex calculateModelIndex(String nodeAndColumnId) {
		...
		nodeAndColumnSplit = new ArrayList<String>(
				Arrays.asList(nodeAndColumnId.split(":")));
	...
	private void adjustToViewport(WTreeViewNode changed) {
		...
		this.validRowCount_ = Math.max(
				0,
				Math.min(this.validRowCount_,
		...
		if (renderMore) {
			int newFirstRenderedRow = Math.min(this.firstRenderedRow_,
					this.getCalcOptimalFirstRenderedRow());
			int newLastValidRow = Math.max(
					lastValidRow,
					Math.min(
							this.rootNode_.getRenderedHeight(),
							this.getCalcOptimalFirstRenderedRow()
				...
				if (c != null) {
					...
					while (c != null && i < node.getChildContainer().getCount()) {
						c = ((node.getChildContainer().getWidget(i)) instanceof WTreeViewNode ? (WTreeViewNode) (node
								.getChildContainer().getWidget(i)) : null);
						if (c != null) {
		...
		if (node.isAllSpacer()) {
			...
			while (topSpacerHeight != 0
				...
				WTreeViewNode n = ((node.getChildContainer().getWidget(1)) instanceof WTreeViewNode ? (WTreeViewNode) (node
						.getChildContainer().getWidget(1)) : null);
				assert n != null;
				WModelIndex childIndex = this.getModel().getIndex(
			...
			while (node.getBottomSpacerHeight() != 0
				...
				WTreeViewNode n = ((node.getChildContainer()
						...
						.getChildContainer().getWidget(lastNodeIndex)) : null);
				assert n != null;
				WModelIndex childIndex = this.getModel().getIndex(
						n.getModelIndex().getRow() + 1, 0, index);
				assert (childIndex != null);
	...
	private WWidget widgetForIndex(final WModelIndex index) {
		...
		if (i != null) {
			...
			WTreeViewNode parentNode = ((parent) instanceof WTreeViewNode ? (WTreeViewNode) (parent)
					: null);
			if (parentNode != null) {
				int row = this.getIndexRow(index, parentNode.getModelIndex(),
						0, Integer.MAX_VALUE);
		...
		if (!(child != null)
			...
			for (int r = 0; r < child.getRow(); ++r) {
				result += this.subTreeHeight(
						this.getModel().getIndex(r, 0, parent), 0, upperBound
								- result);
				if (result >= upperBound) {
					return result;
				}

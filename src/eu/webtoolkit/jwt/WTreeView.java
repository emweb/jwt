...
public class WTreeView extends WAbstractItemView {
	...
	public void setColumnBorder(final WColor color) {
		...
		WApplication.getInstance().getStyleSheet()
				.addRule(this.borderColorRule_);
	}
		...
		if (count != 0 && !(oldCount != 0)) {
			...
			if (useStyleLeft) {
				...
				this.tieRowsScrollJS_
						.setJavaScript("function(obj, event) {Wt3_3_5.getCssRule('#"
								+ this.getId()
		...
		if (nodeAndColumnSplit.size() == 2) {
			...
			try {
				...
				logger.error(new StringWriter()
						.append("WTreeview::onEventItem: bad value for format 1: ")
			...
			if (column != -1) {
				...
				if ((c0index != null)) {
					...
					logger.error(new StringWriter()
							.append("WTreeView::onEventItem: illegal node id: ")

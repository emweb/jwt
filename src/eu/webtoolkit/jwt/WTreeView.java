...
public class WTreeView extends WAbstractItemView {
		...
		if (fixed && !this.column1Fixed_) {
			...
			if (useStyleLeft) {
				this.tieRowsScrollJS_
						.setJavaScript("function(obj, event) {Wt3_1_4.getCssRule('#"
								+ this.getId()
								+ " .Wt-tv-rowc').style.left= -obj.scrollLeft + 'px';}");
			} else {
				this.tieRowsScrollJS_
						.setJavaScript("function(obj, event) {obj.parentNode.style.width = Wt3_1_4.getCssRule('#"
								+ this.getId()
								+ " .cwidth').style.width;$('#"
								+ this.getId()
	...
	private void defineJavaScript() {
		...
		app.doJavaScript("new Wt3_1_4.WTreeView(" + app.getJavaScriptClass()
				+ "," + this.getJsRef() + ","

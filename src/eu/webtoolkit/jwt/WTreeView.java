...
public class WTreeView extends WAbstractItemView {
	...
	public WTreeView(WContainerWidget parent) {
		...
		this.itemTouchEvent_ = new JSignal3<String, String, WTouchEvent>(
		...
		if (count != 0 && !(oldCount != 0)) {
			...
			if (useStyleLeft) {
				...
				this.tieRowsScrollJS_
						.setJavaScript("function(obj, event) {Wt3_5_1.getCssRule('#"
								+ this.getId()
	...
	public void scrollTo(final WModelIndex index,
		...
		final int row = this.getIndexRow(index, this.getRootIndex(), 0,
				Integer.MAX_VALUE);
		WApplication app = WApplication.getInstance();
		if (app.getEnvironment().hasAjax()) {
			...
			s.append("setTimeout(function() { ").append(this.getJsRef())
					.append(".wtObj.scrollTo(-1, ").append(row).append(",")
	...
	protected void render(EnumSet<RenderFlag> flags) {
		...
		if (!EnumUtils.mask(flags, RenderFlag.RenderFull).isEmpty()) {
			...
			if (!this.itemTouchEvent_.isConnected()) {
				this.itemTouchEvent_.addListener(this,
		...
		StringBuilder s = new StringBuilder();
		s.append(this.getJsRef()).append(".wtObj.setRowHeight(")
				.append((int) this.getRowHeight().toPixels()).append(");");
		if (app.getEnvironment().hasAjax()) {
			this.doJavaScript(s.toString());
		}
		...
		if (column == 0) {
			...
			if (this.c0StyleRule_ != null) {
				...
				WApplication.getInstance().getStyleSheet()
						.removeRule(this.c0StyleRule_);
				WApplication.getInstance().getStyleSheet()
						.addRule(this.c0StyleRule_);
			}
		}
		return ci;
	}
	...
	private void defineJavaScript() {
		...
		this.setJavaScriptMember(WT_RESIZE_JS,
				"function(self,w,h) {self.wtObj.wtResize();}");
	}
	...
	private int getIndexRow(final WModelIndex child,
		...
		if (!(child != null)
			...
			if (!(parent == ancestor || (parent != null && parent
					.equals(ancestor)))) {
				return result
						+ 1
						+ this.getIndexRow(parent, ancestor, lowerBound
								- result, upperBound - result);
			} else {
				return result;
			}
		}

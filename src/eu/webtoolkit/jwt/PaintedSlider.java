...
class PaintedSlider extends WPaintedWidget {
	...
	public PaintedSlider(WSlider slider) {
		...
		if (this.slider_.getPositionScheme() == PositionScheme.Static) {
			...
			this.slider_.setOffsets(new WLength(0),
	...
	public void updateState() {
		...
		mouseDownJS.append("obj.setAttribute('down', Wt3_3_10")
				.append(".widgetCoordinates(obj, event).").append(u)
				.append(");").append("Wt3_3_10.cancelEvent(event);");
		...
		mouseMovedJS.append(";").append("var f = objb.onValueChange;")
				.append("if (f) f(vs);");
		...
		this.handleClickedJS_.setJavaScript("function(obj, event) {"
				+ "Wt3_3_10" + ".cancelEvent(event," + "Wt3_3_10"
				+ ".CancelPropagate); }");
		this.update();
	...
	private void onSliderClick(final WMouseEvent event) {
		...
		this.onSliderReleased(this.slider_.getOrientation() == Orientation.Horizontal ? x
				: y);
	}
	...
	private void onSliderReleased(int u) {
		...
		double v = Math.max(
				...
				Math.min(this.slider_.getMaximum(), this.slider_.getMinimum()

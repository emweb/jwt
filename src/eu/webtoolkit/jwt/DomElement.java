...
public class DomElement {
	...
	public void setEvent(String eventName, final String jsCode,
		...
		if (isExposed || anchorClick || jsCode.length() != 0) {
			...
			js.append("o=this;");
			if (anchorClick) {
				js.append("if(e.ctrlKey||e.metaKey||(Wt3_3_11.button(e) > 1))return true;else{");
			}
			js.append(jsCode);
			if (isExposed) {
	...
	public void callMethod(final String method) {
		...
		if (this.var_.length() == 0) {
			this.javaScript_.append("Wt3_3_11").append(".$('").append(this.id_)
					.append("').");
		} else {
	...
	public void removeFromParent() {
		this.callJavaScript("Wt3_3_11.remove('" + this.getId() + "');", true);
	}
	...
	public String asJavaScript(final EscapeOStream out,
		...
		case Delete:
			if (this.javaScriptEvenWhenDeleted_.length() != 0
				...
				if (this.removeAllChildren_ >= 0) {
					this.declare(out);
					if (this.removeAllChildren_ == 0) {
						out.append("Wt3_3_11").append(".setHtml(")
		...
		case Update: {
			...
			if (this.mode_ == DomElement.Mode.ModeUpdate
				...
				if (this.properties_.get(Property.PropertyStyleDisplay) != null) {
					...
					if (style.equals("none")) {
						out.append("Wt3_3_11.hide('").append(this.id_)
								.append("');\n");
						return this.var_;
					} else {
						if (style.length() == 0) {
							out.append("Wt3_3_11.show('").append(this.id_)
									.append("');\n");
							return this.var_;
						} else {
							if (style.equals("inline")) {
								out.append("Wt3_3_11.inline('" + this.id_
										+ "');\n");
								return this.var_;
							} else {
								if (style.equals("block")) {
									out.append("Wt3_3_11.block('" + this.id_
											+ "');\n");
									return this.var_;
								}
							}
			...
			if (this.unwrapped_) {
				out.append("Wt3_3_11.unwrap('").append(this.id_)
						.append("');\n");
			}
			...
			if (this.replaced_ != null) {
				...
				if (this.unstubbed_) {
					out.append("Wt3_3_11.unstub(").append(this.var_)
							.append(',').append(varr).append(',')
			...
			if (!this.childrenToSave_.isEmpty()) {
				this.declare(out);
				out.append("Wt3_3_11").append(".saveReparented(")
						.append(this.var_).append(");");
			}
			...
			for (int i = 0; i < this.childrenToSave_.size(); ++i) {
				out.append("Wt3_3_11.replaceWith('")
	...
	public void declare(final EscapeOStream out) {
		if (this.var_.length() == 0) {
			out.append("var ").append(this.getCreateVar())
					.append("=Wt3_3_11.$('").append(this.id_).append("');\n");
		}
	}
	...
	private void processEvents(WApplication app) {
		...
		if (keypress != null && keypress.jsCode.length() != 0) {
			MapUtils.access(self.eventHandlers_, S_keypress,
					DomElement.EventHandler.class).jsCode = "if (Wt3_3_11.isKeyPress(event)){"
					+ MapUtils.access(self.eventHandlers_, S_keypress,
		...
		if (this.minMaxSizeProperties_
			...
			if (minw != null || maxw != null) {
				if (w == null) {
					...
					expr.append("Wt3_3_11.IEwidth(this,");
					if (minw != null) {
	...
	private void setJavaScriptProperties(final EscapeOStream out,
		...
		for (Iterator<Map.Entry<Property, String>> i_it = this.properties_
			...
			case PropertyAddedInnerHTML:
				...
				out.append("Wt3_3_11.setHtml(").append(this.var_).append(',');
				if (!pushed) {
	...
	private String addToParent(final EscapeOStream out, final String parentVar,
		...
		if (this.type_ == DomElementType.DomElement_TD
			...
			StringBuilder insertJS = new StringBuilder();
			if (pos != -1) {
				insertJS.append("Wt3_3_11.insertAt(").append(parentVar)

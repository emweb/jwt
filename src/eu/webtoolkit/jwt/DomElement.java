...
class DomElement {
	...
	public void removeFromParent() {
		this.callJavaScript("Wt3_1_8.remove('" + this.getId() + "');", true);
	}
	...
	public String asJavaScript(EscapeOStream out, DomElement.Priority priority) {
		...
		case Update: {
			...
			if (this.mode_ == DomElement.Mode.ModeUpdate
				...
				if (this.properties_.get(Property.PropertyStyleDisplay) != null) {
					...
					if (style.equals("none")) {
						out.append("Wt3_1_8.hide('").append(this.id_).append(
								"');\n");
						return this.var_;
					} else {
						if (style.length() == 0) {
							out.append("Wt3_1_8.show('").append(this.id_)
									.append("');\n");
							return this.var_;
						} else {
							if (style.equals("inline")) {
								out.append("Wt3_1_8.inline('" + this.id_
										+ "');\n");
								return this.var_;
							} else {
								if (style.equals("block")) {
									out.append("Wt3_1_8.block('" + this.id_
											+ "');\n");
									return this.var_;
								}
							}
			...
			if (this.unwrapped_) {
				out.append("Wt3_1_8.unwrap('").append(this.id_).append("');\n");
			}
			...
			if (this.replaced_ != null) {
				...
				if (this.unstubbed_) {
					out.append("Wt3_1_8.unstub(").append(this.var_).append(',')
	...
	private void processEvents(WApplication app) {
		...
		if (keypress != null && keypress.jsCode.length() != 0) {
			MapUtils.access(self.eventHandlers_, S_keypress,
					DomElement.EventHandler.class).jsCode = "if (Wt3_1_8.isKeyPress(event)){"
					+ MapUtils.access(self.eventHandlers_, S_keypress,
		...
		if (this.minMaxSizeProperties_
			...
			if (minw != null || maxw != null) {
				if (w == null) {
					...
					expr.append("Wt3_1_8.IEwidth(this,");
					if (minw != null) {
	...
	private void setJavaScriptProperties(EscapeOStream out, WApplication app) {
		...
		for (Iterator<Map.Entry<Property, String>> i_it = this.properties_
			...
			case PropertyAddedInnerHTML:
				out.append("Wt3_1_8.setHtml(").append(this.var_).append(',');
				if (!pushed) {
					escaped
	...
	private void setJavaScriptEvent(EscapeOStream out, String eventName,
		...
		String extra2 = "";
		if (globalUnfocused) {
			extra1 = "var g = event||window.event; var t = g.target||g.srcElement;if ((!t||Wt3_1_8.hasTag(t,'DIV') ||Wt3_1_8.hasTag(t,'BODY') ||Wt3_1_8.hasTag(t,'HTML'))) { ";
			extra2 = "}";
		}
		int fid = nextId_++;
	...
	private String addToParent(EscapeOStream out, String parentVar, int pos,
		...
		if (this.type_ == DomElementType.DomElement_TD
			...
			StringBuilder insertJS = new StringBuilder();
			if (pos != -1) {
				insertJS.append("Wt3_1_8.insertAt(").append(parentVar).append(

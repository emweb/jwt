...
class Line {
	...
	public void moveToNextPage(List<Block> floats, double minX, double maxX,
		...
		for (int i = 0; i < blocks.size(); ++i) {
			...
			if (b.isFloat()) {
				b.layoutFloat(this.y_, this.page_, floats, this.x_,
						this.height_, minX, maxX, false, renderer);
				this.reflow(b);
			} else {
	...
	public void adjustHeight(double height, double baseline,
		...
		if (this.height_ == 0) {
			...
			double descent = Math.max(this.height_ - this.baseline_, height
					- baseline);
	...
	public void finish(AlignmentFlag textAlign, List<Block> floats,
		...
		for (int i = 0; i < this.blocks_.size(); ++i) {
			Block b = this.blocks_.get(i);
			if (b.isFloat()) {
				b.layoutFloat(this.y_ + this.height_, this.page_, floats, minX,
						...
						if (ib.x != LEFT_MARGIN_X) {
							...
							if (b.isText()) {
								for (int k = 0; k < ib.utf8Count; ++k) {
									if (Block.isWhitespace(b.getText().charAt(
											ib.utf8Pos + k))) {
										++ib.whitespaceCount;
									}
								}
								content -= ib.whitespaceWidth
										* ib.whitespaceCount;
								whitespace += ib.whitespaceWidth
										* ib.whitespaceCount;
							}
						} else {
	...
	public void addBlock(Block block) {
		if (this.blocks_.isEmpty()
				|| this.blocks_.get(this.blocks_.size() - 1) != block) {

 ...
 *
 * <p>By providing a custom implementation of this interface, you may use a custom loading indicator
 * using {@link WApplication#setLoadingIndicator(WLoadingIndicator indicator)
 * WApplication#setLoadingIndicator()},instead of the default implementation ({@link
...
public interface WLoadingIndicator {
   ...
   * show() and hide() are stateless slots, and thus you need to make sure that your implementation
   * comforms to that contract, so that it may be optimized to JavaScript (the server-side
   * implementation will only be called during stateless slot prelearning).
   *
   * <p>
   *

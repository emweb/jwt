...
class WClientGLWidget extends WAbstractGLImplementation {
	private static Logger logger = LoggerFactory
			.getLogger(WClientGLWidget.class);
	...
	public void activeTexture(WGLWidget.GLenum texture) {
		...
		do {
			if (debugging_) {
				this.js_
						.append(
								"\n{var err = ctx.getError(); if(err != ctx.NO_ERROR) {alert('error ")
						.append("(unknown)")
						.append(": ' + err); debugger;}}\n");
			}
		} while (false);
	...
	public void bindBuffer(WGLWidget.GLenum target, WGLWidget.Buffer buffer) {
		this.js_.append("ctx.bindBuffer(").append("ctx." + target.toString())
				.append(",").append(buffer.getJsRef()).append(");");
	...
	public void bindTexture(WGLWidget.GLenum target, WGLWidget.Texture texture) {
		this.js_.append("ctx.bindTexture(").append("ctx." + target.toString())
				.append(",").append(texture.getJsRef()).append(");");
	...
	public void blendColor(double red, double green, double blue, double alpha) {
		...
		this.js_.append("ctx.blendColor(").append(WebGLUtils.makeFloat(red))
				.append(",");
	...
	public void blendEquation(WGLWidget.GLenum mode) {
		this.js_.append("ctx.blendEquation(").append("ctx." + mode.toString())
				.append(");");
		do {
			if (debugging_) {
				this.js_
	...
	public void blendFunc(WGLWidget.GLenum sfactor, WGLWidget.GLenum dfactor) {
		this.js_.append("ctx.blendFunc(").append("ctx." + sfactor.toString())
	...
	public void bufferDatafv(WGLWidget.GLenum target,
		...
		for (int i = 0; i < buffer.capacity(); i++) {
			this.js_.append(i == 0 ? "" : ",").append(
					WebGLUtils.makeFloat(buffer.get(i)));
		}
		this.js_.append("])");
	...
	public void clear(EnumSet<WGLWidget.GLenum> mask) {
		...
		if (!EnumUtils.mask(mask, WGLWidget.GLenum.STENCIL_BUFFER_BIT)
				.isEmpty()) {
	...
	public void clearColor(double r, double g, double b, double a) {
		char[] buf = new char[30];
		this.js_.append("ctx.clearColor(").append(WebGLUtils.makeFloat(r))
				.append(",");
	...
	public void clearDepth(double depth) {
		char[] buf = new char[30];
		this.js_.append("ctx.clearDepth(").append(WebGLUtils.makeFloat(depth))
				.append(");");
		do {
			if (debugging_) {
				this.js_
	...
	public WGLWidget.ArrayBuffer createAndLoadArrayBuffer(final String url) {
		WGLWidget.ArrayBuffer retval = new WGLWidget.ArrayBuffer(
				this.arrayBuffers_++);
		this.preloadArrayBuffers_.add(new WClientGLWidget.PreloadArrayBuffer(
				retval.getJsRef(), url));
		return retval;
	}

	public WGLWidget.Framebuffer getCreateFramebuffer() {
		WGLWidget.Framebuffer retval = new WGLWidget.Framebuffer(
				this.framebuffers_++);
	...
	public WGLWidget.Renderbuffer getCreateRenderbuffer() {
		WGLWidget.Renderbuffer retval = new WGLWidget.Renderbuffer(
				this.renderbuffers_++);
	...
	public WGLWidget.Texture createTextureAndLoad(final String url) {
		WGLWidget.Texture retval = new WGLWidget.Texture(this.textures_++);
		this.preloadImages_.add(new WClientGLWidget.PreloadImage(retval
				.getJsRef(), url, 0));
		return retval;
	}
	...
	public void cullFace(WGLWidget.GLenum mode) {
		this.js_.append("ctx.cullFace(").append("ctx." + mode.toString())
				.append(");");
		do {
			if (debugging_) {
				this.js_
	...
	public void depthFunc(WGLWidget.GLenum func) {
		this.js_.append("ctx.depthFunc(").append("ctx." + func.toString())
				.append(");");
		do {
			if (debugging_) {
				this.js_
	...
	public void depthMask(boolean flag) {
		this.js_.append("ctx.depthMask(").append(flag ? "true" : "false")
				.append(");");
		do {
			if (debugging_) {
				this.js_
	...
	public void depthRange(double zNear, double zFar) {
		char[] buf = new char[30];
		this.js_.append("ctx.depthRange(").append(WebGLUtils.makeFloat(zNear))
				.append(",");
	...
	public void enableVertexAttribArray(WGLWidget.AttribLocation index) {
		this.js_.append("ctx.enableVertexAttribArray(")
	...
	public void frontFace(WGLWidget.GLenum mode) {
		this.js_.append("ctx.frontFace(").append("ctx." + mode.toString())
				.append(");");
		do {
			if (debugging_) {
				this.js_
	...
	public void pixelStorei(WGLWidget.GLenum pname, int param) {
		this.js_.append("ctx.pixelStorei(").append("ctx." + pname.toString())
	...
	public void stencilMask(int mask) {
		this.js_.append("ctx.stencilMask(").append(String.valueOf(mask))
				.append(");");
		do {
			if (debugging_) {
				this.js_
	...
	public void stencilOp(WGLWidget.GLenum fail, WGLWidget.GLenum zfail,
		...
		this.js_.append("ctx.stencilOp(").append("ctx." + fail.toString())
				.append(",").append("ctx." + zfail.toString()).append(",")
	...
	public void texImage2D(WGLWidget.GLenum target, int level,
		...
		this.preloadImages_
				.add(new WClientGLWidget.PreloadImage(
	...
	public void texImage2D(WGLWidget.GLenum target, int level,
		...
		int imgNb = this.images_++;
		if (((paintdevice) instanceof WCanvasPaintDevice ? (WCanvasPaintDevice) (paintdevice)
				: null) != null) {
			WCanvasPaintDevice cpd = ((paintdevice) instanceof WCanvasPaintDevice ? (WCanvasPaintDevice) (paintdevice)
					: null);
			...
			if (((paintdevice) instanceof WRasterPaintDevice ? (WRasterPaintDevice) (paintdevice)
				...
				this.preloadImages_.add(new WClientGLWidget.PreloadImage(
						this.currentlyBoundTexture_.getJsRef(), mr.getUrl(),
						imgNb));
			}
		}
		this.js_.append("ctx.texImage2D(").append("ctx." + target.toString())
	...
	public void uniformMatrix2fv(final WGLWidget.UniformLocation location,
		...
		this.js_.append("ctx.uniformMatrix2fv(").append(location.getJsRef())
				.append(",").append(transpose ? "true" : "false").append(",");
		WebGLUtils.renderfv(this.js_, value, 4, JsArrayType.Float32Array);
		this.js_.append(");");
		do {
			if (debugging_) {
	...
	public void uniformMatrix3fv(final WGLWidget.UniformLocation location,
			boolean transpose, double[] value) {
		this.js_.append("ctx.uniformMatrix3fv(").append(location.getJsRef())
				.append(",").append(transpose ? "true" : "false").append(",");
	...
	public void uniformMatrix4fv(final WGLWidget.UniformLocation location,
			boolean transpose, double[] value) {
		this.js_.append("ctx.uniformMatrix4fv(").append(location.getJsRef())
				.append(",").append(transpose ? "true" : "false").append(",");
	...
	public void uniformMatrix4(final WGLWidget.UniformLocation location,
		...
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 4; j++) {
				this.js_.append(i == 0 && j == 0 ? "" : ",").append(
						WebGLUtils.makeFloat(m.getElement(j, i)));
			}
		}
		this.js_.append("])");
		this.js_.append(");");
	...
	public void validateProgram(WGLWidget.Program program) {
		this.js_.append("ctx.validateProgram(").append(program.getJsRef())
				.append(");");
		do {
			if (debugging_) {
				this.js_
	...
	public void vertexAttrib1f(WGLWidget.AttribLocation location, double x) {
		char[] buf = new char[30];
		this.js_.append("ctx.vertexAttrib1f(").append(location.getJsRef())
				.append(",");
		this.js_.append(WebGLUtils.makeFloat(x)).append(");");
		do {
			if (debugging_) {
	...
	public void vertexAttrib2f(WGLWidget.AttribLocation location, double x,
			double y) {
		char[] buf = new char[30];
		this.js_.append("ctx.vertexAttrib2f(").append(location.getJsRef())
				.append(",");
		this.js_.append(WebGLUtils.makeFloat(x)).append(",");
		this.js_.append(WebGLUtils.makeFloat(y)).append(");");
		do {
	...
	public void vertexAttrib3f(WGLWidget.AttribLocation location, double x,
			double y, double z) {
		char[] buf = new char[30];
		this.js_.append("ctx.vertexAttrib3f(").append(location.getJsRef())
				.append(",");
		this.js_.append(WebGLUtils.makeFloat(x)).append(",");
		this.js_.append(WebGLUtils.makeFloat(y)).append(",");
		this.js_.append(WebGLUtils.makeFloat(z)).append(");");
	...
	public void vertexAttrib4f(WGLWidget.AttribLocation location, double x,
			double y, double z, double w) {
		char[] buf = new char[30];
		this.js_.append("ctx.vertexAttrib4f(").append(location.getJsRef())
				.append(",");
		this.js_.append(WebGLUtils.makeFloat(x)).append(",");
		this.js_.append(WebGLUtils.makeFloat(y)).append(",");
		this.js_.append(WebGLUtils.makeFloat(z)).append(",");
	...
	public WGLWidget.JavaScriptMatrix4x4 getCreateJavaScriptMatrix4() {
		javax.vecmath.Matrix4f m = new javax.vecmath.Matrix4f();
		WGLWidget.JavaScriptMatrix4x4 retval = new WGLWidget.JavaScriptMatrix4x4(
				this.matrices_++, JsArrayType.Float32Array, this.glInterface_);
		this.js_.append(retval.getJsRef()).append("=");
		WebGLUtils.renderfv(this.js_, m, JsArrayType.Float32Array);
		this.js_.append(";");
		this.js_.append("obj.jsMatrices[").append(
				String.valueOf(retval.getId())).append("] = ");
		this.js_.append(retval.getJsRef());
		this.js_.append(";");
		do {
			if (debugging_) {
				this.js_
	...
	public void setJavaScriptMatrix4(final WGLWidget.JavaScriptMatrix4x4 jsm,
			final javax.vecmath.Matrix4f m) {
		this.js_.append("Wt3_3_1.glMatrix.mat4.set(");
	...
	public void setClientSideLookAtHandler(
		...
		this.js_.append("obj.setLookAtParams(").append(m.getJsRef()).append(
				",[").append(String.valueOf(centerX)).append(",").append(
				String.valueOf(centerY)).append(",").append(
				String.valueOf(centerZ)).append("],").append("[").append(
				String.valueOf(uX)).append(",").append(String.valueOf(uY))
				.append(",").append(String.valueOf(uZ)).append("],").append(
	...
	public void setClientSideWalkHandler(final WGLWidget.JavaScriptMatrix4x4 m,
		...
		this.js_.append("obj.setWalkParams(").append(m.getJsRef()).append(",")
				.append(String.valueOf(frontStep)).append(",").append(
	...
	public void render(final String jsRef, EnumSet<RenderFlag> flags) {
		if (!EnumUtils.mask(flags, RenderFlag.RenderFull).isEmpty()) {
			...
			tmp.append("{\nvar o = new Wt3_3_1.WGLWidget(").append(
		...
		if (this.updateGL_ || this.updateResizeGL_ || this.updatePaintGL_) {
			...
			this.js_ = new StringWriter();
			tmp.append("}\n");
			final boolean preloadingSomething = this.preloadImages_.size() > 0
	...
	static class PreloadArrayBuffer {
		private static Logger logger = LoggerFactory
				.getLogger(PreloadArrayBuffer.class);
	...
	private static void renderiv(final Writer os, final java.nio.IntBuffer a,
			WGLWidget.GLenum type) {
		try {
			switch (type) {
			case BYTE:
			...
			for (i = 0; i < a.capacity(); i++) {
				os.append(i == 0 ? "" : ",").append(
						WebGLUtils.makeInt(a.get(i)));
			}
			os.append("])");
	...
	private static void renderiv(final Writer os, int[] a, int size,
			WGLWidget.GLenum type) {
		try {
			switch (type) {
			case BYTE:
	...
	private static boolean debugging_ = true;
}

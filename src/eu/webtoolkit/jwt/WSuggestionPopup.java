...
public class WSuggestionPopup extends WCompositeWidget {
	...
	private void defineJavaScript() {
		...
		app.doJavaScript("new Wt3_1_9.WSuggestionPopup("
	...
	static String wtjs2(WApplication app) {
		String s = "function(s,f,w,C,q,x){function y(c){var g=c.value;c=c.selectionStart?c.selectionStart:g.length;for(var d=w?g.lastIndexOf(w,c-1)+1:0;d<c&&C.indexOf(g.charAt(d))!=-1;)++d;return{start:d,end:c}}this.match=function(c){var g=y(c),d=c.value.substring(g.start,g.end),m=\"^\";if(q.length!=0)m=\"(^|(?:[\"+q+\"]))\";m+=\"(\"+d.replace(new RegExp(\"([\\\\^\\\\\\\\\\\\][\\\\-.$*+?()|{}])\",\"g\"),\"\\\\$1\")+\")\";m=new RegExp(m,\"gi\");return function(o){if(!o)return d;var t=false; if(d.length){var z=o.replace(m,\"$1\"+s+\"$2\"+f);if(z!=o){t=true;o=z}}return{match:t,suggestion:o}}};this.replace=function(c,g,d){g=y(c);var m=c.value.substring(0,g.start)+d+x;if(g.end<c.value.length)m+=c.value.substring(g.end,c.value.length);c.value=m;if(c.selectionStart){c.selectionStart=g.start+d.length+x.length;c.selectionEnd=c.selectionStart}}}";
		if ("ctor.WSuggestionPopupStdMatcher".indexOf(".prototype") != -1) {
			return "Wt3_1_9.ctor.WSuggestionPopupStdMatcher = " + s + ";";
		} else {
			if ("ctor.WSuggestionPopupStdMatcher".substring(0, 5).compareTo(
					"ctor.".substring(0, 5)) == 0) {
				return "Wt3_1_9."
						+ "ctor.WSuggestionPopupStdMatcher".substring(5)
						+ " = " + s + ";";
			} else {
				return "Wt3_1_9.ctor.WSuggestionPopupStdMatcher = function() { ("
						+ s + ").apply(Wt3_1_9, arguments) };";
			}
		}
	}
	...
	static String instantiateStdMatcher(WSuggestionPopup.Options options) {
		...
		s.append("new Wt3_1_9.WSuggestionPopupStdMatcher(").append(

	...
	public WSuggestionPopup(String matcherJS, String replacerJS,
		...
		this.editKeyDown_
				.setJavaScript("function(edit, event) {  var self = "
						+ this.getJsRef()
						+ ";  var sel = self.getAttribute('sel');  if (sel != null) sel = Wt2_99_2.getElement(sel);  if (self.style.display != 'none' && sel != null) {    if ((event.keyCode == 13) || (event.keyCode == 9)) {      sel.firstChild.onclick();      Wt2_99_2.cancelEvent(event);      return false;    } else if (event.keyCode == 40 || event.keyCode == 38) {      if (event.type.toUpperCase() == 'KEYDOWN')        self.setAttribute('kd', 'true');      if (event.type.toUpperCase() == 'KEYPRESS'        && self.getAttribute('kd') == 'true') {        Wt2_99_2.cancelEvent(event);        return false;      }      var n = sel;      for (var n = (event.keyCode == 40) ? n.nextSibling : n.previousSibling;            n != null && n.nodeName.toUpperCase() == 'DIV' && n.style.display == 'none';           n = (event.keyCode == 40) ? n.nextSibling : n.previousSibling) { }      if (n != null && n.nodeName.toUpperCase() == 'DIV') {        sel.setAttribute('class', null);        n.setAttribute('class', 'sel');        self.setAttribute('sel', n.id);      }      return false;    }  }  return (event.keyCode != 13 && event.keyCode != 9);}");
		this.editKeyUp_
				.setJavaScript("function(edit, event) {  var self = "
						+ this.getJsRef()
						+ ";  var sel = self.getAttribute('sel');  if (sel != null) sel = Wt2_99_2.getElement(sel);  if (event.keyCode == 27 || event.keyCode == 37 || event.keyCode == 39) {    self.style.display = 'none';    if (event.keyCode == 27)      edit.blur();  } else {    var text = edit.value;    var matcher = "
						+ this.matcherJS_

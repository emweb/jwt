 ...
 * &quot;Wt-invalid&quot; when the validator returns not {@link }. Therefore, it
 * is unlikely you will need this class anymore.</i>
 * <p>
 ...
 * state of the input.
 * <p>
 * Visual feed-back may be given by showing an invalidStateWidget when input is
 * invalid, an invalidEmptyStateWidget when the input is invalid because
 * mandatory and empty, or a validStateWidget when input is valid. All of these
 * widgets may be 0, indicating that no widget will be shown for the
 * corresponding state.
 * <p>
...
public class WValidationStatus extends WCompositeWidget {
	...
	public WValidationStatus(WFormWidget field, WWidget validStateWidget,
		...
		if (this.validStateWidget_ != null) {
			...
			this.validStateWidget_
					.setHidden(this.state_ != WValidator.State.Valid);
		}
		if (this.invalidStateWidget_ != null) {
			this.impl_.addWidget(this.invalidStateWidget_);
			this.invalidStateWidget_
					.setHidden(this.state_ != WValidator.State.Invalid);
		}
		if (this.invalidEmptyStateWidget_ != null) {
			this.impl_.addWidget(this.invalidEmptyStateWidget_);
			this.invalidEmptyStateWidget_
					.setHidden(this.state_ != WValidator.State.InvalidEmpty);
		}
		field.changed().addListener(this, new Signal.Listener() {
			public void trigger() {
				WValidationStatus.this.inputChanged();
			}
		});
		field.keyWentUp().addListener(this, new Signal1.Listener<WKeyEvent>() {
	 ...
	 * This signal gets emitted when the state changes from {@link } to {@link },
	 * or from {@link } to {@link }.
	 */
	...
	private void inputChanged() {
		...
		if (state != this.state_) {
			if (this.validStateWidget_ != null) {
				this.validStateWidget_
						.setHidden(state != WValidator.State.Valid);
			}
			if (this.invalidStateWidget_ != null) {
				this.invalidStateWidget_
						.setHidden(state != WValidator.State.Invalid);
			}
			if (this.invalidEmptyStateWidget_ != null) {
				this.invalidEmptyStateWidget_
						.setHidden(state != WValidator.State.InvalidEmpty);
			}
			if (this.state_ == WValidator.State.Valid
					|| state == WValidator.State.Valid) {
				this.state_ = state;

...
public class WAxisSliderWidget extends WPaintedWidget {
   ...
   * Creates an axis slider widget.
   *
   * <p>Calls {@link #WAxisSliderWidget(WContainerWidget parent) this((WContainerWidget)null)}
   */
   ...
   * Creates an axis slider widget.
   *
   * <p>Calls {@link #WAxisSliderWidget(WDataSeries series, WContainerWidget parent) this(series,
   ...
   *
   * <p>Labels are enabled by default.
   */
  ...
  protected void paintEvent(WPaintDevice paintDevice) {
    ...
    for (int i = 0; i < this.getChart().getAxis(Axis.XAxis).segments_.size(); ++i) {
      segmentsBak.add(new WAxis.Segment(this.getChart().getAxis(Axis.XAxis).segments_.get(i)));
    }
    ...
    {
      double u =
          -this.getChart().xAxis_.transformHandle.getValue().getDx()
              / (chartArea.getWidth() * this.getChart().xAxis_.transformHandle.getValue().getM11());
      selectionRect = new WRectF(0, top, maxW, h - (top + bottom));
      this.transform_.setValue(
          new WTransform(
              1 / this.getChart().xAxis_.transformHandle.getValue().getM11(),
              0,
              0,
              1,
              u * maxW,
    ...
    if (horizontal) {
      ...
      painter.strokePath(line, this.getChart().getAxis(Axis.XAxis).getPen());
    } else {
      axis.render(
          painter,
          axisProperties,
    ...
    if (this.getMethod() == WPaintedWidget.Method.HtmlCanvas) {
      WApplication app = WApplication.getInstance();
      StringBuilder ss = new StringBuilder();
      ss.append("new Wt3_5_2.WAxisSliderWidget(")
    ...
    for (int i = 0; i < segmentsBak.size(); ++i) {
      this.getChart().getAxis(Axis.XAxis).segments_.add(new WAxis.Segment(segmentsBak.get(i)));
    }

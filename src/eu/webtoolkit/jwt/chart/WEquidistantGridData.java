 ...
 * for this dataseries does not contain an x- and y-axis. Instead the class
 * derives the x- and y-values from the minimum and delta provided in the
 * constructor. The size of the model determines the size of the grid. The model
 * itself is structured as a table, where every value represents the z-value of
 * a data-point. The corresponding x- and y-values are calculated by adding
 * delta times the row/column-index to the axis-minimum.
 */
public class WEquidistantGridData extends WAbstractGridData {
	private static Logger logger = LoggerFactory
			.getLogger(WEquidistantGridData.class);

	/**
	 * Constructor.
	 */
	public WEquidistantGridData(WAbstractItemModel model, double XMin,
			double deltaX, double YMin, double deltaY) {
		super(model);
		this.XMinimum_ = XMin;
		this.deltaX_ = deltaX;
		this.YMinimum_ = YMin;
			...
			if (axis == Axis.YAxis_3D) {
				...
				if (axis == Axis.ZAxis_3D) {
					...
					throw new WException(
							"WEquidistantGridData.C: unknown Axis-type");
				}
			}
		}
	}
	...
	public WString axisLabel(int u, Axis axis) {
		if (axis == Axis.XAxis_3D) {
			...
			if (axis == Axis.YAxis_3D) {
				...
				throw new WException(
						"WEquidistantGridData: don't know this type of axis");
			}
		}
	}
	...
	void pointDataFromModel(final java.nio.ByteBuffer simplePtsArray,
		...
		for (int i = 0; i < Nx; i++) {
			scaledXAxis
					.putFloat((float) ((this.XMinimum_ + i * this.deltaX_ - xMin) / (xMax - xMin)));
		}
		for (int j = 0; j < Ny; j++) {
			scaledYAxis
					.putFloat((float) ((this.YMinimum_ + j * this.deltaY_ - yMin) / (yMax - yMin)));
		}
		for (int i = 0; i < Nx; i++) {
			for (int j = 0; j < Ny; j++) {
				if ((this.model_.getData(i, j,
			...
			for (; i < (k + 1) * SURFACE_SIDE_LIMIT + 1; i++) {
				...
				for (; j < Ny; j++) {
					...
					simplePtsArrays.get(bufferIndex).putFloat(
							scaledYAxis.getFloat(4 * (j)));
					simplePtsArrays.get(bufferIndex).putFloat(
							(float) ((StringUtils.asNumber(this.model_.getData(
									i, j)) - zMin) / (zMax - zMin)));
				}
				cnt1++;
			}

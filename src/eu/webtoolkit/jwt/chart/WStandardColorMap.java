 ...
 * WStandardColorMap has two modes: a continuous mode, in which the colors are
 * linearly interpolated in between the pair values, and a non-continuous mode,
 * where the values are not interpolated, so that the colormap has a banded
 * effect. In non-continuous mode, the color of a given point P is the color of
 * the pair with the largest value smaller than P.
 * <p>
...
public class WStandardColorMap extends WAbstractColorMap {
	private static Logger logger = LoggerFactory
			.getLogger(WStandardColorMap.class);
	 ...
	 * >ColorBrewer</a>, which contains lots of useful info and example schemes
	 * you might want to use for your custom colormaps.
	 */
	public WStandardColorMap(double min, double max, boolean continuous) {
		...
		this.colors_.add(new WStandardColorMap.Pair(this.min_, new WColor(255,
				255, 178)));
		this.colors_.add(new WStandardColorMap.Pair(this.min_ + 1 * interval,
				new WColor(254, 204, 92)));
		this.colors_.add(new WStandardColorMap.Pair(this.min_ + 2 * interval,
				new WColor(253, 141, 60)));
		this.colors_.add(new WStandardColorMap.Pair(this.min_ + 3 * interval,
				new WColor(240, 59, 32)));
		this.colors_.add(new WStandardColorMap.Pair(this.min_ + 4 * interval,
				new WColor(189, 0, 38)));
	}
	 ...
	 */
	public WStandardColorMap(double min, double max,
			final List<WStandardColorMap.Pair> colors, boolean continuous) {
		super(min, max);
		this.continuous_ = continuous;
		this.colors_ = new ArrayList<WStandardColorMap.Pair>();
		...
		for (int i = 0; i < colors.size(); i++) {
			...
			if (val < prev) {
				throw new WException(
						"WStandardColorMap: the provided vector is not sorted");
			}
			prev = val;
		}
	...
	public WColor toColor(double value) {
		...
		if (this.continuous_) {
			...
			double factor = (value - mapVal1.getValue())
					/ (mapVal2.getValue() - mapVal1.getValue());
			return this.interpolate(mapVal1.getColor(), mapVal2.getColor(),
					factor);
		} else {
	...
	public void paintLegend(WPainter painter, final WRectF area) {
		...
		int stripWidth = 50;
		this.createStrip(painter, new WRectF(0, (int) (textHeight / 2 + 0.5),
				(int) stripWidth, (int) (height - textHeight)));
		painter.setPen(new WPen());
		painter.setBrush(new WBrush());
		painter.drawRect(new WRectF(0.5, (int) (textHeight / 2) + 0.5,
				stripWidth, height - textHeight));
		painter.translate(stripWidth, textHeight / 2);
		if (this.continuous_) {
			int lineHeights = (int) (height / textHeight);
			int lhPerTick = 1 + this.tickSpacing_;
			int nbTicks = lineHeights % lhPerTick == 0 ? lineHeights
					/ lhPerTick : lineHeights / lhPerTick + 1;
			...
			for (int i = 0; i < nbTicks; i++) {
				...
				painter.drawText(10, -textHeight / 2, 40, textHeight,
						EnumSet.of(AlignmentFlag.AlignLeft,
								AlignmentFlag.AlignMiddle), StringUtils
								.asString(value, this.format_.toString()));
				value -= valDiff;
				if (rest > 0) {
			...
			painter.drawLine(0, 0.5, 4, 0.5);
			painter.drawText(10, -textHeight / 2, 100, textHeight, EnumSet.of(
			...
			for (int i = nbTicks - 1; i >= 0; i--) {
				double relPos = -(this.colors_.get(i).getValue() - this.max_)
						/ (this.max_ - this.min_);
				...
				painter.drawText(10, -textHeight / 2, 40, textHeight,
						EnumSet.of(AlignmentFlag.AlignLeft,
								AlignmentFlag.AlignMiddle), StringUtils
								.asString(this.colors_.get(i).getValue(),
										this.format_.toString()));
	...
	private WColor interpolate(final WColor color1, final WColor color2,
			double factor) {
		return new WColor((int) ((1 - factor) * color1.getRed() + factor
				* color2.getRed()),
				(int) ((1 - factor) * color1.getGreen() + factor
						* color2.getGreen()), (int) ((1 - factor)
						* color1.getBlue() + factor * color2.getBlue()));
	}

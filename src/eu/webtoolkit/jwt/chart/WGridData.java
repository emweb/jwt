 ...
 * for this dataseries is structured as a table. One of the columns (by default
 * index 0) contains the x-axis values, one of the rows (by default index 0)
 * contains the y-axis values. All other values in the table contain the z-value
 * corresponding to the x- and y-values with the same column- and row-index.
 */
public class WGridData extends WAbstractGridData {
	...
	public double minimum(Axis axis) {
		if (axis == Axis.XAxis_3D) {
			if (this.YAbscisRow_ != 0) {
				return StringUtils.asNumber(this.model_.getData(0,
						this.XAbscisColumn_));
			} else {
				return StringUtils.asNumber(this.model_.getData(1,
						this.XAbscisColumn_));
			}
		} else {
			if (axis == Axis.YAxis_3D) {
				...
				if (axis == Axis.ZAxis_3D) {
					...
					throw new WException(
							"WAbstractGridData.C: unknown Axis-type");
				}
			}
		}
	}
		...
		if (axis == Axis.XAxis_3D) {
			...
			if (axis == Axis.YAxis_3D) {
				...
				if (this.XAbscisColumn_ != this.model_.getColumnCount()) {
					return StringUtils
							.asNumber(this.model_.getData(this.YAbscisRow_,
									this.model_.getColumnCount() - 1));
				} else {
					return StringUtils
							.asNumber(this.model_.getData(this.YAbscisRow_,
									this.model_.getColumnCount() - 2));
				}
			} else {
				if (axis == Axis.ZAxis_3D) {
					if (!this.rangeCached_) {
	...
	public WString axisLabel(int u, Axis axis) {
		if (axis == Axis.XAxis_3D) {
			...
			return StringUtils.asString(this.model_.getData(u,
					this.XAbscisColumn_));
		} else {
			if (axis == Axis.YAxis_3D) {
				...
				return StringUtils.asString(this.model_.getData(
						this.YAbscisRow_, u));
			} else {
	...
	void pointDataFromModel(final java.nio.ByteBuffer simplePtsArray,
		...
		for (int i = 0; i < nbModelRows; i++) {
			...
			scaledXAxis.putFloat((float) ((StringUtils.asNumber(this.model_
					.getData(i, this.XAbscisColumn_)) - xMin) / (xMax - xMin)));
		}
		for (int j = 0; j < nbModelCols; j++) {
			if (j == this.XAbscisColumn_) {
				continue;
			}
			scaledYAxis.putFloat((float) ((StringUtils.asNumber(this.model_
					.getData(this.YAbscisRow_, j)) - yMin) / (yMax - yMin)));
		}
		int rowOffset = 0;
		int colOffset = 0;
		for (int i = 0; i < nbModelRows; i++) {
			...
			for (int j = 0; j < nbModelCols; j++) {
				...
				if ((this.model_.getData(i, j,
						ItemDataRole.MarkerBrushColorRole) == null)) {
					simplePtsArray.putFloat(scaledXAxis
							.getFloat(4 * (i - rowOffset)));
					simplePtsArray.putFloat(scaledYAxis
							.getFloat(4 * (j - colOffset)));
					simplePtsArray
							.putFloat((float) ((StringUtils
									.asNumber(this.model_.getData(i, j,
											ItemDataRole.DisplayRole)) - zMin) / (zMax - zMin)));
					if (!(this.model_.getData(i, j,
							ItemDataRole.MarkerScaleFactorRole) == null)) {
						simplePtsSize.putFloat((float) StringUtils
								.asNumber(this.model_.getData(i, j,
					...
					coloredPtsArray.putFloat(scaledXAxis
							.getFloat(4 * (i - rowOffset)));
					coloredPtsArray.putFloat(scaledYAxis
							.getFloat(4 * (j - colOffset)));
					coloredPtsArray
							.putFloat((float) ((StringUtils
					...
					if (!(this.model_.getData(i, j,
							ItemDataRole.MarkerScaleFactorRole) == null)) {
						coloredPtsSize.putFloat((float) StringUtils
								.asNumber(this.model_.getData(i, j,
										ItemDataRole.MarkerScaleFactorRole)));
					} else {
	...
	void surfaceDataFromModel(final List<java.nio.ByteBuffer> simplePtsArrays) {
		...
		bufferIndex = (nbXaxisBuffers - 1) * nbYaxisBuffers
				+ (nbYaxisBuffers - 1);
		int i = (nbXaxisBuffers - 1) * SURFACE_SIDE_LIMIT;
		rowOffset = 0;
		for (; i < nbModelRows - 1; i++) {
			...
			for (; j < nbModelCols - 1; j++) {
				...
				simplePtsArrays
						...
						.putFloat(
								(float) ((StringUtils.asNumber(this.model_
										.getData(i + rowOffset, j + colOffset)) - zMin) / (zMax - zMin)));
			}
		}
	}
			...
			for (int j = 0; j < nbModelCols - 1; j++) {
				...
				if ((this.model_.getData(i + rowOffset, j + colOffset,
					...
					simplePtsArrays.get(simpleBufferIndex).putFloat(
							scaledYAxis.getFloat(4 * (j)));
					simplePtsArrays.get(simpleBufferIndex).putFloat(z0);
					double modelVal = StringUtils.asNumber(this.model_.getData(
							i + rowOffset, j + colOffset));
					float delta = modelVal <= 0 ? zeroBarCompensation : 0;
					simplePtsArrays.get(simpleBufferIndex)
							.putFloat(
					...
					double modelVal = StringUtils.asNumber(this.model_.getData(
							i + rowOffset, j + colOffset));
					float delta = modelVal <= 0 ? zeroBarCompensation : 0;
					coloredPtsArrays.get(coloredBufferIndex)
							.putFloat(
									(float) ((modelVal - zMin) / (zMax - zMin))
											+ delta);

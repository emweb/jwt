...
class WLegend3D extends WLegend {
	...
	public void renderLegend(WPainter painter,
			final List<WAbstractDataSeries3D> dataseries) {
		...
		for (int i = 0; i < dataseries.size(); i++) {
			...
			if (series.getColorMap() == null) {
				...
				painter.fillRect(0, offset, labelWidth, textHeight, new WBrush(
						seriesColor));
			} else {
				series.getColorMap().createStrip(painter,
						new WRectF(0, offset, labelWidth, textHeight));
			}
			painter.drawText(labelWidth + 10, 0, 100, lineHeight, EnumSet.of(
					AlignmentFlag.AlignLeft, AlignmentFlag.AlignMiddle), series
					.getTitle());
			if (count == this.legendColumns_) {
				painter.translate(-(this.legendColumns_ - 1)
						* this.legendColumnWidth_.getValue(), lineHeight);
				count = 0;
			} else {

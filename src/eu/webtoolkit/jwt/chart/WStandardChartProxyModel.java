...
/**
 * A {@link WAbstractChartModel} implementation that wraps a
 * {@link WAbstractItemModel}.
 * <p>
 * 
...
public class WStandardChartProxyModel extends WAbstractChartModel {
	private static Logger logger = LoggerFactory
			.getLogger(WStandardChartProxyModel.class);

	/**
	 * Creates a new {@link WStandardChartProxyModel} that wraps the given
	 * source model.
	 */
	public WStandardChartProxyModel(WAbstractItemModel sourceModel,
		...
		sourceModel.columnsInserted().addListener(this,
		...
		sourceModel.columnsRemoved().addListener(this,
				new Signal3.Listener<WModelIndex, Integer, Integer>() {
					public void trigger(WModelIndex e1, Integer e2, Integer e3) {
						WStandardChartProxyModel.this.sourceModelModified();
					}
				});
		sourceModel.rowsInserted().addListener(this,
				new Signal3.Listener<WModelIndex, Integer, Integer>() {
					public void trigger(WModelIndex e1, Integer e2, Integer e3) {
						WStandardChartProxyModel.this.sourceModelModified();
					}
				});
		sourceModel.rowsRemoved().addListener(this,
				new Signal3.Listener<WModelIndex, Integer, Integer>() {
					public void trigger(WModelIndex e1, Integer e2, Integer e3) {
						WStandardChartProxyModel.this.sourceModelModified();
					}
				});
		sourceModel.dataChanged().addListener(this,
				new Signal2.Listener<WModelIndex, WModelIndex>() {
					public void trigger(WModelIndex e1, WModelIndex e2) {
						WStandardChartProxyModel.this.sourceModelModified();
					}
				});
		sourceModel.headerDataChanged().addListener(this,
				new Signal3.Listener<Orientation, Integer, Integer>() {
					public void trigger(Orientation e1, Integer e2, Integer e3) {
						WStandardChartProxyModel.this.sourceModelModified();
					}
				});
		sourceModel.layoutChanged().addListener(this, new Signal.Listener() {
			public void trigger() {
				WStandardChartProxyModel.this.sourceModelModified();
			}
		});
		sourceModel.modelReset().addListener(this, new Signal.Listener() {
			public void trigger() {
				WStandardChartProxyModel.this.sourceModelModified();
			}
		});
	 ...
	 * <p>
	 * Calls
	 * {@link #WStandardChartProxyModel(WAbstractItemModel sourceModel, WObject parent)
	...
	public double getData(int row, int column) {
		return StringUtils.asNumber(this.sourceModel_.getData(row, column,
	...
	public WString getDisplayData(int row, int column) {
		return StringUtils.asString(this.sourceModel_.getData(row, column,
				ItemDataRole.DisplayRole));
	}
	...
	public WString getHeaderData(int column) {
		return StringUtils.asString(this.sourceModel_.getHeaderData(column,
				Orientation.Horizontal, ItemDataRole.DisplayRole));
	}

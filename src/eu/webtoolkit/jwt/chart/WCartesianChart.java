...
public class WCartesianChart extends WAbstractChart {
	...
	/**
	 * Enable on-demand loading TODO(Roel): elaborate.
	 * <p>
	 * 
		...
		if (this.isInteractive() || this.hasDeferredToolTips_) {
			...
			ss.append("minZoom:{x:")
					.append(MathUtils.roundJs(this.getAxis(Axis.XAxis)
							.getMinZoom(), 3)).append(",y:[");
			for (int i = 0; i < this.getYAxisCount(); ++i) {
				if (i != 0) {
					ss.append(',');
				}
				ss.append(MathUtils.roundJs(this.yAxis(i).getMinZoom(), 3));
			}
			ss.append("]},");
			ss.append("maxZoom:{x:")
					.append(MathUtils.roundJs(this.getAxis(Axis.XAxis)
	...
	private void createPensForAxis(Axis ax, int yAxis) {
		...
		for (int i = 1;; ++i) {
			...
			if (this.isOnDemandLoadingEnabled() && ax == Axis.XAxis
					&& i > level + 1) {
				break;
			}

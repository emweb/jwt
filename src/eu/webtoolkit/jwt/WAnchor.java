 ...
 * must use {@link setTarget(TargetNewWindow)}. Even for non-HTML documents,
 * this may be important since pending Ajax requests are cancelled if documents
 * are not served within the browser window in certain browsers.
 * <p>
 * WAnchor is an {@link inline} widget.
 * <p>
 * <p>
 * <i><b>Note: </b>If you set a text or image using one of the API methods like
 * {@link } or {@link } or a constructor, you should not attempt to remove all
 * contents (using {@link }, or provide a layout (using {@link }), as this will
 * result in undefined behaviour: the text or image are simply inserted as
 * widgets into the container.</i>
 * </p>
 * <h3>CSS</h3>
 * <p>
...
public class WAnchor extends WContainerWidget {
	 ...
	 * <p>
	 * When the link points to a {@link resource}, the contents of the link may
	 * be generated by your application on demand.
	 * <p>
	 * When the link points to an {@link internal path}, activating the anchor
	 * will change the {@link application&apos;s internal path} or open a new
	 * session with the given path as {@link initial path}). This is the easiest

 ...
 * {@link Token} obtained through OpenID Connect Authentication. These Claims
 * are normally represented by a JSON object that contains a collection of name
 * and value pairs for the Claims.
 * <p>
 * One can use setScopeToken to map claims to a scopeToken. The value of these
 * claims will be retrieved using the
 ...
 * >http://openid.net/specs/openid-connect-core-1_0.html#UserInfo</a> for more
 * information.
 * <p>
...
public class OidcUserInfoEndpoint extends WResource {
	private static Logger logger = LoggerFactory
			.getLogger(OidcUserInfoEndpoint.class);
	...
	/**
	 * Retrieves the set of claims that has been mapped to the given scope
	 * token.
	 */
	 ...
	 * @see AbstractUserDatabase#idpJsonClaim(User user, String claim)
	 */
	protected com.google.gson.JsonObject generateUserInfo(final User user,
		...
		for (Iterator<String> claim_it = claims.iterator(); claim_it.hasNext();) {
			...
			com.google.gson.JsonElement claimValue = this.db_.idpJsonClaim(
					user, claim);

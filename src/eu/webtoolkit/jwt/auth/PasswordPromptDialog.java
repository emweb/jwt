 ...
 * <p>
 * 
 * This is a simple dialog, useful for prompting the user to enter his password.
 * This may be convenient for example to let the user upgrade from a weak
 * authentication to a strong authentication.
 * <p>
 * The dialog uses a {@link Login} object to get the currently identified user,
 * and also sets the result of the login process by calling
 * {@link Login#login(User user, LoginState state) Login#login()} on this
 * object.
 * <p>
 * The dialog renders the
 * <code>&quot;Wt.Auth.template.password-prompt&quot;</code> template.
 */
public class PasswordPromptDialog extends WDialog {
	private static Logger logger = LoggerFactory
			.getLogger(PasswordPromptDialog.class);
	...
	protected void check() {
		...
		if (this.model_.validate()) {
			...
			this.impl_.updateViewField(this.model_, AuthModel.PasswordField);
			WPushButton okButton = (WPushButton) this.impl_
					.resolveWidget("ok-button");

 ...
 * specification</a>), which can be used to allow the user to be safely
 * authenticated with your web application without needing to store or even
 * handle his authorization credentials (such as a password).
 * <p>
 ...
 * REST-like manner.
 * <p>
 * This implementation only supports authentication using the Authorization Code
 * Flow.
 * <p>
 * The configuration of this service is done by using the setters the service
 * class exposes. Before the authentication process can be started these
 * settings must be configured first and may not be changed afterwards.
 * <p>
 * The OpenID Connect protocol, including the subsequent use for authentication,
 * consists of a number of consecutive steps, some of which require user
 * interaction, and some which require the use of remote web services. The state
 * machine for this process is implemented in an {@link OidcProcess}. To use
 * OpenID Connect, you need to create such a process and listen for state
 * changes.
 */
public class OidcService extends OAuthService {
	 ...
	 * <p>
	 * The service needs to be correctly configured before being able to call
	 * this function. configure() needs to be called first to check if the

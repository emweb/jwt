 ...
 * <p>
 * 
 * This class will process the environment and perform the authorization of the
 * user if this is possible. If this is successful, an authorization code will
 ...
 * has to be a valid redirect URI where the user will be redirected to when the
 * authorization has been succesful. &quot;scope&quot;, which has to be set to
 * the scope of the requested information. &quot;response_type&quot;, which has
 * to be set to &quot;code&quot;. If the &quot;state&quot; parameter has been
 * included, it will be passed on as a paremeter to the redirect URI.
 * <p>
 * When the client ID and the redirect URI is valid but something else went
 * wrong, an &quot;error=invalid_request&quot; will be sent to the client. If
 * the user failed to log in correctly &quot;error=login_required&quot; will be
 * sent. If everything went OK, the &quot;code&quot; parameter is included which
 * can be used to obtain a token from a token endpoint.
 * <p>
 * See <a
 * href="https://tools.ietf.org/rfc/rfc6749.txt">https://tools.ietf.org/rfc
 * /rfc6749.txt</a> for more information.
 * <p>
 ...
 * {@link AbstractUserDatabase#idpClientRedirectUris(OAuthClient client)
...
public class OAuthAuthorizationEndpointProcess extends WObject {
	private static Logger logger = LoggerFactory
			.getLogger(OAuthAuthorizationEndpointProcess.class);

	/**
	 * Constructor.
	 */
	public OAuthAuthorizationEndpointProcess(final Login login,
			final AbstractUserDatabase db) {
		super();
		this.db_ = db;
	...
	public void processEnvironment() {
		...
		if (!(clientId != null)) {
			logger.error(new StringWriter().append(
					"Missing client_id parameter.").toString());
			return;
		}
		...
		String prompt = WApplication.getInstance().getEnvironment()
				.getParameter("prompt");
	...
	/**
	 * Returns true if the request was a valid OAuth request with the correct
	 * parameters.
	 */

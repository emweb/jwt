...
class StdGridLayoutImpl2 extends StdLayoutImpl {
	...
	static WJavaScriptPreamble appjs1() {
		return new WJavaScriptPreamble(
				...
				"layouts2",
				"new (function(){var D=[],B=false,J=this,P=false;this.find=function(r){return jQuery.data(document.getElementById(r),\"layout\")};this.setDirty=function(r){(r=jQuery.data(r,\"layout\"))&&r.setDirty()};this.setChildLayoutsDirty=function(r,E){var y,w;y=0;for(w=r.descendants.length;y<w;++y){var v=r.descendants[y];if(E){var A=r.WT.getElement(v.getId());if(!r.WT.contains(E,A))continue}v.setDirty()}};this.add=function(r){function E(y,w){var v,A;v=0;for(A=y.length;v< A;++v){var u=y[v];if(u.getId()==w.getId()){y[v]=w;w.descendants=u.descendants;return}else if(u.contains(w)){E(u.descendants,w);return}else if(w.contains(u)){w.descendants.push(u);y.splice(v,1);--v;--A}}y.push(w)}E(D,r);J.scheduleAdjust()};var Q=false;this.scheduleAdjust=function(){if(!Q){Q=true;setTimeout(function(){J.adjust()},0)}};this.adjust=function(r,E){function y(v,A){var u,I;u=0;for(I=v.length;u<I;++u){var H=v[u];y(H.descendants,A);A==1&&P&&H.setDirty();H.measure(A)}}function w(v,A){var u, I;u=0;for(I=v.length;u<I;++u){var H=v[u];if(H.apply(A))w(H.descendants,A);else{v.splice(u,1);--u;--I}}}if(r){(r=this.find(r))&&r.setItemsDirty(E);J.scheduleAdjust()}else{Q=false;if(!B){B=true;y(D,0);w(D,0);y(D,1);w(D,1);P=B=false}}};this.updateConfig=function(r,E){(r=this.find(r))&&r.setConfig(E)};window.onresize=function(){P=true;J.scheduleAdjust()}})");
	}

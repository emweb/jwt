...
class StdGridLayoutImpl2 extends StdLayoutImpl {
	...
	static WJavaScriptPreamble appjs1() {
		return new WJavaScriptPreamble(
				...
				"layouts2",
				"new (function(){var F=[],A=false,L=this,M=false;this.find=function(r){return jQuery.data(document.getElementById(r),\"layout\")};this.setDirty=function(r){(r=jQuery.data(r,\"layout\"))&&r.setDirty()};this.add=function(r){function G(D,B){var x,z;x=0;for(z=D.length;x<z;++x){var u=D[x];if(u.contains(B)){G(u.descendants,B);return}else if(B.contains(u)){B.descendants.push(u);D.splice(x,1);--x;--z}}D.push(B)}G(F,r)};var N=false;this.scheduleAdjust=function(){if(!N){N= true;setTimeout(function(){L.adjust()},0)}};this.adjust=function(r,G){function D(x,z){var u,H;u=0;for(H=x.length;u<H;++u){var E=x[u];D(E.descendants,z);z==1&&M&&E.setDirty();E.measure(z)}}function B(x,z){var u,H;u=0;for(H=x.length;u<H;++u){var E=x[u];if(E.apply(z))B(E.descendants,z);else{x.splice(u,1);--u;--H}}}if(r)(r=this.find(r))&&r.setItemsDirty(G);else{N=false;if(!A){A=true;D(F,0);B(F,0);D(F,1);B(F,1);M=A=false}}};this.updateConfig=function(r,G){(r=this.find(r))&&r.setConfig(G)};window.onresize= function(){M=true;L.scheduleAdjust()}})");
	}

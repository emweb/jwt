 ...
 * <p>
 * Use {@link } to configure the textFormat of the text. The default textFormat
 * is {@link }, which allows XHMTL markup to be included in the text. Tags and
 ...
 * <p>
 * WText is by default {@link inline}, unless the XHTML contents starts with an
 * element such as <code>&lt;div&gt;</code>, <code>&lt;h&gt;</code> or
 * <code>&lt;p&gt;</code> that is displayed as a block, in which case the widget
 * will also display as a block.
 * <p>
 * <h3>CSS</h3>
 * <p>
...
public class WText extends WInteractWidget {
	 ...
	 * (not created using {@link }) and it could not be parsed as valid XML. In
	 * that case the textFormat is set to {@link }.
	 * <p>
	 * Therefore, if you wish to use {@link }, but cannot be sure about
	 * <code>text</code> being valid XML, you should verify that the {@link } is
	 ...
	 * created using {@link }), then if the <code>text</code> could not be parsed
	 * as valid XML, the textFormat is changed to {@link }.
	 * <p>
	 * Therefore, if you wish to use {@link }, but cannot be sure about
	 * <code>text</code> being valid XML, you should verify that the {@link } is
	 * {@link } after construction.
	 ...
	 * created using {@link }), it is parsed using an XML parser which discards
	 * malicious tags and attributes silently. When the parser encounters an XML
	 * parse error, the textFormat is changed to {@link }.
	 * <p>

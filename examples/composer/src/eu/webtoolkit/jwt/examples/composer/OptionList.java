...
public class OptionList extends WContainerWidget
{
	...
	/**
	 * Update the stateless implementations after an Option has been
	 * hidden or shown.
	 */
	public void update()
	{
	 ...
	 */
	private Option optionNeedReset_;
	...
	void optionVisibilityChanged(Option opt, boolean hidden)
	{
		...
		for (int i = options_.size() - 1; i > 0; --i)
		{
			if (options_.get(i) == opt)
			{
				...
				if (!options_.get(i).isHidden())
					break;
		}

		/*
		 * The Option to the right needs to relearn its stateless slot code for
		 * hide() and show().
		 */
		for (int i = 0; i < options_.size(); ++i)
		{
			if (options_.get(i) == opt)
			{
				for (int j = i + 1; j < options_.size(); ++j)
				{
					if (!options_.get(j).isHidden())
					{
						optionNeedReset_ = options_.get(j);
						break;
					}
				}

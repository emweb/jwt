...
public class AttachmentEdit extends WContainerWidget
{
	...
	public AttachmentEdit(Composer composer, WContainerWidget parent)
	{
		...
		remove_.setMargin(new WLength(5, WLength.Unit.Pixel), EnumSet.of(Side.Left));
		remove_.clicked().addListener(this, new Signal1.Listener<WMouseEvent>()
		{
		...
		upload_.fileTooLarge().addListener(this, new Signal1.Listener<Integer>()
		{
	...
	/**
	 * Update the file now. Returns whether a new file will be
	 * uploaded. If so, the uploadDone signal will be signalled when the file is
	...
	/**
	 * Signal emitted when a new attachment has been uploaded (or
	 * failed to upload.
	 */
	...
	private void uploaded()
	{
		if (!upload_.isEmptyFileName())
		{
			...
			uploaded_.setText(escapeText(fileName_) + " (<i>" + contentDescription_ + " </i>) " + size);
			uploaded_.show();

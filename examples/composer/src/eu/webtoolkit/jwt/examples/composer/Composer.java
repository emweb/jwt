...
public class Composer extends WCompositeWidget
{
	...
	/**
	 * Get the list of attachments. The ownership of the attachment
	 * spool files is transferred to the caller as well, be sure to delete them !
	 */
	...
	/**
	 * Slot attached to the Send button. Tries to save the mail
	 * message, and if successful, sends it.
	 */
	private void sendIt()
	{
		if (!sending_)
	...
	/**
	 * Slot attached to the Save now button. Tries to save the mail
	 * message, and gives feedback on failure and on success.
	 */
	...
	/**
	 * Slot attached to the Discard button. Discards the current
	 * message: emits the discard event.
	 */
	...
	/**
	 * Slotcalled when an attachment has been uploaded. This used
	 * during while saving the email and waiting for remaining attachments to be
	 * uploaded. It is connected to the AttachmentEdit control signals that are
	 * emitted when an attachment has been processed.
	 */
	void attachmentDone()
	{
		if (saving_)
	...
	private void createUi()
	{
		...
		statusMsg_.setMargin(new WLength(15, WLength.Unit.Pixel), EnumSet.of( Side.Left));
		...
		edits_.getElementAt(0, 0).resize(new WLength(1, WLength.Unit.Percentage), new WLength());

		/*
		 * To, Cc, Bcc
		 */
		toEdit_ = new AddresseeEdit("msg.to", edits_.getElementAt(0, 1), edits_.getElementAt(0, 0));
		// add some space above To:
		edits_.getElementAt(0, 1).setMargin(new WLength(5), EnumSet.of(Side.Top));
		ccEdit_ = new AddresseeEdit("msg.cc", edits_.getElementAt(1, 1), edits_.getElementAt(1, 0));
		bccEdit_ = new AddresseeEdit("msg.bcc", edits_.getElementAt(2, 1), edits_.getElementAt(2, 0));
		...
		subject_.resize(new WLength(99, WLength.Unit.Percentage), new WLength());
		...
		attachOtherFile_ = new Option("msg.attachanother", edits_.getElementAt(5, 1));
		attachOtherFile_.hide();
		 ...
		 * Button events.
		 */
		topSendButton_.clicked().addListener(this, new Signal1.Listener<WMouseEvent>()
		{
		...
		botSendButton_.clicked().addListener(this, new Signal1.Listener<WMouseEvent>()
		{
		...
		topSaveNowButton_.clicked().addListener(this, new Signal1.Listener<WMouseEvent>()
		{
		...
		botSaveNowButton_.clicked().addListener(this, new Signal1.Listener<WMouseEvent>()
		{
		...
		topDiscardButton_.clicked().addListener(this, new Signal1.Listener<WMouseEvent>()
		{
		...
		botDiscardButton_.clicked().addListener(this, new Signal1.Listener<WMouseEvent>()
		{
		...
		addbcc_.clicked().addListener(this, new Signal1.Listener<WMouseEvent>()
				{
					public void trigger(WMouseEvent a)
					{
		 ...
		 */
		attachFile_.clicked().addListener(this, new Signal1.Listener<WMouseEvent>()
		{
			public void trigger(WMouseEvent a)
			{
		...
		attachOtherFile_.clicked().addListener(this, new Signal1.Listener<WMouseEvent>()
		{
			public void trigger(WMouseEvent a)
			{
				attachments_.get(attachments_.size() - 1).show();
	...
	/**
	 * All attachments have been processed, determine the result of
	 * saving the message.		addcc_.clicked.addListener(addcc_.doHide);
	 */
	private void saved()
	{
		/*

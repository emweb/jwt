...
class Validators extends ControlsWidget {
	...
	public Validators(EventDisplayer ed) {
		...
		addText(tr("validators-intro"), this);
		...
		le.setValidator(iv);
		this.fields_.add(new Pair<WFormWidget, WText>(le, addText("", table
				.getElementAt(0, 2))));
		addText("<tt>WDoubleValidator</tt>: range [-5.0 to 15.0]", table
				.getElementAt(1, 0));
		le = new WLineEdit(table.getElementAt(1, 1));
		le.setValidator(new WDoubleValidator(-5, 15));
		this.fields_.add(new Pair<WFormWidget, WText>(le, addText("", table
				.getElementAt(1, 2))));
		addText("<tt>WDateValidator</tt>, default format \"yyyy-MM-dd\"", table
				.getElementAt(2, 0));
		le = new WLineEdit(table.getElementAt(2, 1));
		le.setValidator(new WDateValidator());
		this.fields_.add(new Pair<WFormWidget, WText>(le, addText("", table
				.getElementAt(2, 2))));
		addText("<tt>WDateValidator</tt>, format \"dd-MM-yy\"", table
				.getElementAt(3, 0));
		...
		this.fields_.add(new Pair<WFormWidget, WText>(le, addText("", table
				.getElementAt(4, 2))));
		addText("<tt>WLengthValidator</tt>, 6 to 11 characters", table
				.getElementAt(5, 0));
		...
		addText("<tt>WRegExpValidator</tt>, IP address", table.getElementAt(6,
				0));
		...
		this.fields_.add(new Pair<WFormWidget, WText>(ta, addText("", this)));

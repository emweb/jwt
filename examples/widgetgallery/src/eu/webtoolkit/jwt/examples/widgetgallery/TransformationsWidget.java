...
class TransformationsWidget extends WPaintedWidget {
	private static Logger logger = LoggerFactory
			.getLogger(TransformationsWidget.class);
	...
	protected void paintEvent(WPaintDevice paintDevice) {
		...
		for (int i = 0; i < 2; i++) {
			...
			this.drawFilledPolygon(painter,
					new WColor(0, 255, 0, 255 - i * 200));
			painter.restore();
		}
		...
		for (int ring = 1; ring < 6; ring++) {
			painter.save();
			painter.setBrush(new WBrush(new WColor(51 * ring, 255 - 51 * ring,
					255)));
	...
	private void drawFilledPolygon(WPainter painter, WColor color) {
		...
		WPointF[] points = { new WPointF(20, 0), new WPointF(60, 0),
				...
				new WPointF(20, 69.2), new WPointF(0, 34.6), new WPointF(20, 0) };
